...
In this work we examine two prominent classes of control algorithms: value-based methods and actor-critic methods. 
...
In contrast to value based methods, policy gradient methods explicitly parameterize and learn a policy $\pi_\theta$ with parameters $\theta$. Policy gradient methods perform policy improvement by directly estimating % todo: math for derivative of objective wrt policy parameters. 
and applying a standard gradient update directly to $\theta$. While % todo: math for derivative of objective wrt policy parameters. 
cannot be computed analytically due to its dependence on $\dynamics$, which we assume is unknown, the policy gradient theorem \tocite{sutton} yields a value proportional to the true gradient that can be sampled from trajectories of experience:
% todo: math for policy gradient. (just a single equation j(theta) = = \mathbb{E}_{\pi} \left[ G_t \frac{\nabla \pi(A_t \mid S_t, \boldsymbol{\theta})}{\pi(A_t \mid S_t, \boldsymbol{\theta})} \right]
Intuitively, moving $\theta$ in this direction increases the probability of actions with high value. An advantage of this approach over value based methods is that % todo: complete this sentence concisely explaining how learning a policy can be easier than learning a value function and we don't actually need to know a perfectly accurate value function to get good behavior or improve the policy. 
Another benefit is that directly parameterizing $\pi$ allows for easily learning polices for continuous action spaces. 
% todo: simple and concise explaination of how PG methods also allow for learning policies for continuous action spaces and why this doesn't work with value based methods.
Actor-critic methods are a special class of policy gradient methods that learn both a state or state-action value function (the critic) and an explicitly parameterized policy (the actor). The critic can be learned just as with value-based methods, using any of the targets for policy evaluation presented in \tocite{ref to pred section}. These methods can then use a bootstrapped target from the critic in place of $G_t$ in the policy gradient estimate, greatly reducing variance at the cost of some additional bias. 
% todo same as the PG theorem above but with \qparam in place of G_t and an apprixmately equal symbol. 
Note that $r_{t+1} + \gamma \vparam(s_{t+1})$ may also be used in place of $\qparam$ if a state-value function is learned instead. In fact, any of the bootstrapped targets discussed in section \tocite{ref: td eval section above} may be used, though the TD(0) target is most common. This approach is simply another instance of GPI: The critic evaluates the actor and the actor is improved by increasing the probability of actions with high value estimates from the critic. 

Some actor-critic methods use the advantage function \tocite{citation}, defined as % todo: inline definiton of the advantage function
in place of $\qparam$ in \tocite{ref PG theorem with q above}, which further reduces variance without introducing any additional bias in the policy gradient estimate \cite{sutton1998introduction}.
% todo: def of policy gradient with the advantage
The critic can be updated just as with value-based methods, using any of the targets for policy evaluation presented in \tocite{ref to pred section}. A simple actor-critic algorithm that learns $\vparam$ and uses \tocite{ref: eq of policy gradient with the advantage given above} to estimate the policy gradient is shown below:
% todo: pseudocode for AC with regular advantage function. Use v for critic. 
...
Proximal Policy Optimization (PPO) \tocite{PPO} is an actor-critic method designed for use with DNNs. The core algorithm is the same actor-critic method in % todo: ref actor critic method 
but using a $\lambda$-return in place of the TD(0) target in both the critic and actor updates. Analogously to how DQN makes Q-learning more robust, PPO introduces several changes to address two distinct sources of instability for actor-critic methods in deep RL.
...
Lastly, PPO employs many additional algorithmic tools for stabilizing training such as input normalization and gradient clipping, all of which are outlined in \tocite{37 implementation details}. We give pseudocode containing only the core algorithmic components discussed here:
% todo: ppo pseudocode. 
