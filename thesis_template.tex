...
We introduce a new algorithm that employs a parallel search to avoid sequential backtracking and common initialization heuristics \cite{vaswani2019painless,galli2023monotone}. % todo: footnote mentioning that PALS was developed by Samuel Neumann in joint work. 
...
Note that the exponential moving average over the chosen step size may also be applied to traditional backtracking methods, which we do in this work to increase robustness to high variance gradient estimates in RL for all line search methods. % todo: make not run on. 
...
\todo{!Important Question for Adam!: Related to the Q above, I am now realizing that I think it makes more sense for this method to update every timestep rather than only update every n steps based on curvature for a single batch, which seems potentially bad. Should it update every step to be an exact baseline from the literature or average to be directly comparable with GRASS and make sure that GRASS is good because it doesn't normalize the gradient, rather than being good bc of the averaging of stats? Not sure what is best here. Maybe good to ablate as mentoned above}
