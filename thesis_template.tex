...
shows that with small batch size in Name This Game, the accepted step sizes are roughly 3 orders of magnitude larger than the tuned baseline and performance flatlines, a pattern that holds in the other environments as well \todo{for Adam: do i need to show this with a full small batch size plot grid in the appendix or ok just saying?}. At the larger batch size, step sizes are much closer to the fixed baseline, though GRASS appears to select more conservative step sizes approximately two orders of magnitude smaller than the baseline. % todo: make cleaner ie i dont want to repeat "the baseline" twice
Examining the return in figure % todo ref to fig
, we find that despite the improved performance with large batch size, PALS still suffers from performance degradation roughly a third of the way though the run, while GRASS outperforms both tuned baselines. This stark difference in step size and resulting performance between small and larger batch size suggests that adaptive methods may suffer from overfitting to the high variance % opus todo: make this clear and concise. confirm it makes sense. Not sure overfitting is the right word. 
...
% PPO
GRASS, GRASS Cauchy actually appear much more robust to this change than the other adaptive methods, achieving the stongest performance at batch size 64 and 
\begin{figure}[htbp]
    \centering
