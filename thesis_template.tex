...
GRASS can also utilize second order information by identifying the Cauchy point along the raw update vector and then scaling the update by $\eta$, rather than first normalizing the update vector to length $\eta$. % todo: does this make sense / clear enough
This approach, which we refer to as GRAS
...
\section{Algorithms}
We compare the performance of GRASS, GRASS Cauchy, the first-order trust region method outlined in % local todo: ref to section
(henceforth refered to as simply Trust Region), the Cauchy point method (denoted TR Cauchy), PALS, backtracking Armijo linesearch, and tuned fixed step size baselines.
...
Across all experiments, for ?? and ?? we use standard values for from the classical optimization literature \cite{nocedal2006numerical}: and , respectively. % local todo: values for these hypers from the GRASS pseudocode
For % local todo: values for the update interval
we use 1/4th % local todo: make latex for 1/4th
of the target network update interval for DQN and 1/4th of the total updates performed after each data collection phase for PPO. All the examined trust-region-based methods utilize the median gain-ratio introduced in GRASS. However, we present an ablation using the traditional sinlge-step gain-ratio with all methods in % local todo: ref a new appdix section for this
