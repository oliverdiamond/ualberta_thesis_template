...
Reinforcement learning methods often learn estimates of $\vfn \in \mathbb{R}^n$ and/or $\qfn \in \mathbb{R}^m$, as the true values are typically unknown to the agent. We define these estimates as parameterized functions $\vparam$ and $\qparam$ where $\vparams$ and $\qparams$ are learnable parameters. If $|\stateSpace|$ (or $|\stateSpace \times \actionSpace|$ in the case of action-value estimation) is relatively small, $\vparams$ and $\qparams$ can be the entries of tables with one row per state or state-action pair, allowing for exact estimation of $\vfn$ and $\qfn$, respectively. This is known as the \textit{tabular} setting, which underlies many of the foundational theoretical results in RL. 

However, for problems in which $\stateSpace$ is massive, like navigating a robot from visual inputs where each discrete state represents a unique combination of pixels, maintaining a table is not possible due to memory constraints. The algorithm can therefore only learn an approximation of $\vfn$ or $\qfn$. In this case, the designer specifies a set of $k$ feature functions $\varphi_1, \dots, \varphi_k$, each mapping states or state-action pairs to a scalar $\varphi_i : \stateSpace \to \RR$, that allow for generalizing value estimates over large subsets of $\stateSpace$. We refer to the set of all feature vectors the agent may observe as the observation space $\obsSpace \subseteq \RR^k$. The designer may also specify a set of $d$ feature functions $\psi_1, \dots, \psi_d$, each mapping actions to a scalar $\psi_j : \actionSpace \to \RR$, where the concatenation of these features $\psi \doteq (\psi_1, \dots, \psi_d) : \actionSpace \to \RR^d$ is an injection. While this does not alias multiple actions to the same representation, it provides a semantic structure for generalizing action-value estimates over $\actionSpace$ in problems where actions are more intuitively thought of as continuous. The same may also be true for $\stateSpace$ when $\varphi \doteq (\varphi_1, \dots, \varphi_k) : \stateSpace \to \RR^d$ is an injection. 

This setting, known as RL with function approximation (FA), has two important variants: linear FA and non-linear FA. In the linear case, the features serve as a fixed-basis representation for an approximator that is linear in $\vparams$ or $\qparams$, while in the non-linear case the approximator is non-linear in $\vparams$ or $\qparams$. The tabular setting is actually a special case of linear FA with $|\stateSpace|$ features corresponding to indicator functions for each state $s \in \stateSpace$ (the generalization for tabular estimation of $\qfn$ is analogous). Because of its generality and importance in modern RL, we present all algorithms in this work for the FA setting.
