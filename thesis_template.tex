...
we observe that step sizes are roughly 2-4 orders of magnitude smaller than the tuned baselines, and roughly 1-2 orders of magnitude smaller than the chosen step sizes in the other Atari environments, with the exception of the divergent curves for Battle Zone. In Figure ?b % \todo{add fig of step size and sharpness for qbert}% todo: add ref to env after i add 
...
once again GRASS and line search methods fall within an order of magnitude of the baseline while the Trust Region methods have larger step sizes. Noteably, no methods exhibit the divergent nor overly conservative step sizes observed in some Atari environments with DQN.
We hypothesize that this is primarily a consequence of PPO, rather than of the environments, for three reasons. First, unlike the single step Q-learning loss used by DQN, the critic targets in PPO are computed using $\lambda$-returns, which are less dependent on current value estimates, preventing inflation when the targets are reset after each data collecton phase. Second, PPO is an on-policy algorithm, implying that actions with inflated value estimates from the critic will actually be sampled by the learned policy % todo: latex math for this consistent with the background section
...
shows that with small batch size in Name This Game, the accepted step sizes are roughly 3 orders of magnitude larger than the tuned baseline, and performance flatlines. This pattern holds in all environments, as shown in Appendix \todo{Grid of plots with large batch size in the appendix}. % todo: ref appendix
...
To examine this hypothesis, Figure % todo: ref to fig 
shows an approximation of the loss landscape on several indivudal batches sampled at different points in learning with PALS when using %todo: latex for adam update without momentum and adam udpate with momentum
to select step size. We construct these approximations using the evaluated loss for each candidate step size in the line search. We find that \todo{finish this once you make the plot}
...
\end{figure}
\todo{note for adam: this is a much smaller difference then in atari but ive ablated this a long time ago and found that using a much larger window still didnt make a difference}. In Figure % todo: ref fig
we also observe that step sizes remain largely unaffected by this change, as demonstrated for Humanoid in Figure % todo: ref fig. 
