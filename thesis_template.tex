...
% todo: make below into equation not inline and also add line below expectation giving the full expanded double summaton equatilty from sutton and barto. 
$\vfn(s) = \mathbb{E}_\pi \left[ R_{t+1} + \gamma \vfn(S_{t+1}) \mid S_t = s \right] $
$\qfn(s, a) = \mathbb{E}_\pi \left[ R_{t+1} + \gamma \qfn(S_{t+1}, A_{t+1}) \mid S_t = s, A_t = a \right] $
...
Deep Q-Learning (DQN) \tocite{dqn} is a seminal deep RL algorithm that was introduced to stabalize Q-learning when paired with modern DNNs for function approximation. The algorithm addresses two significant issues that arise from naively using a neural network for function approxmiation in \tocite{q learning algo pseudocode above}. 
...
% Todo: make sure that I am not missing anything in this section, since I am much less familiar with optimization than RL. Go back and check nocedal and wright numerical optimization LS and TR chapters. Double check there are not critical omissions or mistakes. Also note where this is being too concise and could be expanded for clarity's sake. 
Having introduced the % todo: lead in the smoothly transitions from deep rl to here
We now turn to line search methods, the first of the two globalization strategies for adaptive step size examined in this work. 

We first define the stochastic minimization problem  $\xstar = \argmin_{x \in \mathbb{R}^n} f_k(x)$, where $f_k$ is an unbiased stochastic estimate of an objective function $f$. Iterative algorithms typically update the current estimate $\xAt{k}$ by selecting an update direction $\updateAt{k}$ and taking a step in this direction, such that $\xAt{k+1} = \xAt{k} + \stepSizeAt{k} \updateAt{k}$ where $\stepSizeAt{k} > 0$ is a scalar step size. While $\stepSizeAt{k}$ is often fixed or decayed, recent work has moved toward adaptive step size selection to improve convergence and robustness \tocite{citation}. 

\subsection{Backtracking Armijo Line Search}
A prominent class of adaptive methods is line search \cite{nocedal2006numerical,armijo1966minimization,vaswani2019painless,vaswani2025armijo,galli2023monotone,fox2024nonmonotone,fox2024glocal}. These methods search for an optimal $\stepSizeAt{k}$ along $\updateAt{k}$. % todo: think there should be a more specific or formal way of introducing this
% todo: do i need to introduce backtracking as a special kind of LS here?
The stochastic Armijo line search selects an $\stepSizeAt{k}$ that satisfies the \textit{stochastic Armijo condition} \cite{vaswani2019painless}:
\begin{equation}
        f_k(\xAt{k} + \stepSizeAt{k} \updateAt{k}) \le f_k(\xAt{k}) + c \stepSizeAt{k} \inner{\nabla f_k(\xAt{k})}{\updateAt{k}},
    \label{eq:armijo_condition}
\end{equation}
where $c \in (0, 1)$ is a hyperparameter controlling the conservativeness of the condition and $\inner{\nabla f_k(\xAt{k})}{\updateAt{k}} < 0$, implying that $\updateAt{k}$ must be a descent direction for $f_k$. Line search methods are flexible in that they easily be layered atop any optimizer that produces an update vector $\updateAt{k}$ satisfying this property \cite{nocedal2006numerical,vaswani2025armijo,shea2024so}.
...
\subsection{Parallel Argmin Line Search} \label{sec: pals}
