...
\todo{I think I wrote too much here or some of this could be better served in the intro}
...
% todo: def of MSVE for state vals 
or when learning action values
% todo: def of MSVE for action vals
In the tabular setting, if $\dynamics$ and $\rewardFunction$ are known, this objective can be minimized by iteratively solving the system of Bellman equations in \todo{ref bellman eq from above} via dynamic programming \cite{sutton1998introduction}.\todo{should I formally introduce DP and GPI? My initial intention was to avoid this for brevity but I ended up doing in depth in other areas anyways}. When $\dynamics$ and $\rewardFunction$ are unknown, policy evaluation becomes an online stochastic optimization problem \todo{citation}, as algorithms must construct estimates of $\vfn$ and $\qfn$ from stochastic trajectories and then minimize the MSE. Once the update targets are obtained, any optimization algorithm may be employed to reduce the loss, however all pseudocode presented in the the next two sections uses stochastic gradient descent (SGD). 
...
% todo: action value-form of td(0) update
and replacing %todo: inline latex for value of the next action 
with % todo: inline latex for max over the action values in $S'$ 
...
As the policy used for gathering data differs from the greedy policy being evaluated, Q-learning is classified as an \textit{off-policy} learning algorithm. It allows for the sustained exploration needed for convergence in the tabular setting, while maintaining the ability to learn $\pi^*$ via greedy policy updates. A key benefit of off-policy learning is the ability to learn from experience generated by any policy, which proves essential in its use with deep neural networks (DNNs) discussed in % todo: {DQN section reference}. 
...
\For{each episode}
    ...
    \While{$S$ is not terminal}
        ...
        \State Every $C$ steps, set $\qparams' \gets \qparams$ % todo: this should just be a condition of if the timestep mod c is zero with a timeestep counter. 
        \State $S \gets S'$
    \EndWhile
\EndFor
\end{algorithmic}
...
\label{alg:ppo}
\begin{algorithmic}[1]
\State \textbf{Input:} clip parameter $\epsilon$, number of epochs $K$, initial actor weights $\theta$, initial critic weights $\vparams$
\For{each iteration}
...
Equation \ref{eq:adam_armijo_condition}. Non-monotone line searches often choose larger step sizes than Armijo line searches, potentially leading to flatter, more robust minima \cite{fox2024nonmonotone}.
...
\chapter{Reduction Ratio for Adaptive Step Size}
...
\chapter{Experiment material from paper}
