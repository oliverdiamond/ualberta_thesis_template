...
shows the chosen step sizes for all methods in the Acrobot environment. We observe that chosen step sizes for GRASS, GRASS Cauchy and the line search methods all lie within an order of magnitude below the fixed step size baseline. Step sizes for standard Trust Region and TR Cauchy are much larger, as for these algorithms the step size for the raw update vector is effectively % todo: length of tr radius times 1/update vector norm. make sure notion consistent with math in background and methods chapters
\todo{Q For Adam: If we wanted to make make everything on the same scale we could report norm of the displacement in weight space for each method instead, thoughts?}
The larger step size for regular armijo relative to PALS reflects the bias of backtracking methods twoards larger step sizes, even when smaller ones may achieve equal or greater reductions in the objective. Plots of return and step size for all environments are given in appendix % todo: ref appendix
...
. For example, in Battle Zone, we observe that all methods except GRASS exhibit either a total failure or a clear collapse in return after some period of learning progress. In Figure ?a \todo{Plot of step size and q values in battle zone} % todo: add ref to plot once i put it in
...
we observe that step sizes are roughly 2-4 orders of magnitude smaller than the tuned baselines, and roughly 1-2 orders of magnitude smaller than the chosen step sizes in the other Atari environments. In Figure ?b % \todo{add fig of step size and sharpness for qbert}% todo: add ref to env after i add 
...
This suggests that as observed in the supervised learning, globalization strategies may be susceptible to overly conservative step sizes resulting in convergence to sharp, poorly-generalizing local minima. We also observe sharpness for the baseline methods in all Atari environments does appear to converge to roughly $2/\eta$, as observed for RMSProp in supervised learning \tocite{citation}. Yet, interestingly, these methods utilize the Adam optimizer, for which convergence to $3.8/\eta$ is expected \tocite{citation}.
...
shows that chosen step sizes for linesearch methods and the GRASS variants lie roughly within an order of magnitude of the tuned step size
...
% However, we do not observe this behavior with the Armijo line search in the online control setting, perhaps because the implicit regularization of stochastic updates may alleviate EOS undershooting \cite{roulet2024stepping}.

% the strong performance of PALS relative to regular backtracking may be attributed to selecting more conservative step sizes in the RL control setting, where stochastic estimates of non-stationary objectives can be high-variance.
