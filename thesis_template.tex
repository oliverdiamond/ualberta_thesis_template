...
This thesis builds upon unpublished work done in collaboration with Samuel Neumann, Chen Fan, Martha White and Adam White.
...
In classical optimization literature, two common methods of selecting step size are backtracking line search and trust region methods \cite{nocedal2006numerical}. These two classes of algorithms are known as globalization strategies because they guarantee convergence to a stationary point from any starting point in the full-batch, supervised setting.
...
\item \textbf{Sensitivity to sharp loss.} Line search methods have been shown to sometimes select highly conservative step sizes in the full-batch supervised setting \tocite{roulet2024stepping}, resulting in slow convergence to sharp local minima with poor generalization performance. This phenomenon has also been observed in the deep RL prediction setting \cite{PALS2026}. We demonstrate this issue can also occur with line search and trust-region in control and propose Sharpness Aware Minimization \tocite{SAM} as a potential remedy.
\item \textbf{High computational overhead.} \todo{change this to introduce grass and pals}While several methods have proposed heuristics to reduce backtracking in line-search methods by adaptively setting the starting point \cite{vaswani2019painless}, the worst-case complexity of these methods remains linear in the number of backtracking steps, and some heuristics are not applicable to deep RL. The Parallel Argmin Line Search (PALS) \cite{PALS2026} addresses this fault by evaluating candidate step sizes in parallel. However, its higher energy demands and the lack of perfect constant-time scaling on consumer hardware could limit its viability for real-world learning on edge devices. Traditional trust-region methods rely on expensive second-order local models, making them impractical for large neural networks. To overcome these limitations, we introduce a lightweight, first-order approach based on trust-region. Our method matches or exceeds the performance of all examined adaptive step size baselines and is generally competitive with tuned fixed step sizes, all while only requiring only one additional forward pass per update compared to a fixed step size.
\end{enumerate}
...
\begin{equation}
    ...
    \label{eq:tr_model}
\end{equation}
where $H_k$ is a symmetric matrix and $p$ is a candidate step. The next step is obtained by minimizing this model within a ball of radius $\Delta_k > 0$, referred to as the trust region radius. 
\begin{equation}
...
Similarly to the EMA used in PALS, aggregating the gain-ratio over many updates before performing an adjustment can be done with any trust region method, not just GRASS. To isolate for the effect of removing the implicit normalization step in trust region methods, we also test the median gain-ratio with a simple first-order trust region method, which like GRASS utilizes a first-order model with an arbitrary descent direction in place of $\nabla f_k(\xAt{k})$.
...
For the line search methods, we use a sufficient decrease coefficient of $c=0.1$, which is a commonly used with stochastic line search in supervised learning \cite{vaswani2019painless}.
...
\todo{make sure to note the merits of PALS relative to regular line search}
In this chapter, we examine the performance of all the examined adaptive methods with DQN and PPO. For DQN, we identify all adaptive methods perform well in the classic control suite, demonstrating that globalization strategies can serve as an effective means of automatic step size selection in deep RL. Moving to more complex environments, we find that GRASS greatly outperforms the other adaptive methods with DQN in Atari. However, GRASS still struggles relative to the tuned baseline with smaller batch size, as globalization strategies appear to exhibit several distinct failure modes in this setting. With PPO, we find that all methods perform similarly well in classic control, while GRASS is the best performing method when used with PPO in Mujoco.
...
We first examine the use of globalization strategies for adaptive step size with DQN in classic control. As shown in Figure % todo: ref fig once I add
, all the examined methods perform roughly equally in all three environments. Figure \todo{add fig of acrobot step size} % todo: ref fig once i add
shows the chosen step sizes for all methods in the Acrobot environment. Note that we use $\stepSize$ and $\Delta_k$ of Section~\ref{subsec:trust_region} interchangeably, as $\Delta_k$ serves the role of the step size in the update for GRASS and in the normalized update for the first-order trust region method. Similarly, for the Cauchy point method we report step size as $\tau_k \Delta_k$. At the beginning of learning, step sizes for GRASS and the trust region methods quickly rise from their starting point $\trustRegionInit$ to a reasonable value for the environment and then decay thereafter. Step sizes for standard Trust Region and the Cauchy point are larger because the effective step size for these methods is actually $\Delta_k / \norm{\updateAt{k}}$ and $\tau_k \Delta_k / \norm{\updateAt{k}}$, respectively. \todo{Q For Adam: If we wanted to make everything on the same scale we could report norm of the displacement in weight space for each method instead, thoughts? We could also plot the technical step size applied to the raw update which would have a 1/norm(u) term but i did not log the unnormalized norm(u) so would have to rerun a bunch of stuff :/ } The search methods exhibit a similar pattern, though the larger step size for regular armijo relative to PALS reflects the bias of backtracking methods twoards larger steps. Plots of return and step size for all environments are given in appendix % todo: ref appendix

Turning to more complex environments, we find the results are much more nuanced. 
Figure todo{add fig of atari normalized bars} % todo: ref fig once i add 
shows normalized performance across the Atari-5, where GRASS greatly outperformed all other adaptive step size methods. While GRASS performed better than Tuned-512, which uses the same batch size, the other methods failed to match this baseline. However, GRASS still performed worse on average than Tuned-32. Examining the learning curves in Figure \todo{add fig of atari learning curves} % % todo: ref fig once i add 
we see that performance in the Atari-5 is much less uniform across methods and environments than in the classic control domains. For example, in Battle Zone, we observe that all adaptive methods except GRASS exhibit either a total failure or a clear collapse in return after some period of learning progress. Figure ? \todo{Plot of step size and q values and loss in battle zone} % todo: add ref to plot once i put it in
shows these collapses in performance are correlated with steadily increasing step size, loss, gradient norms, and Q-values. 

This finding suggests globalization strategies may be prone to accelerating the effects of the deadly triad. Both the Armijo condition and the gain-ratio only measure if a step reduced the current objective by a sufficient fraction of the predicted decrease from a local model. Neither method accounts for the fact that larger steps may also inflate the Q-values from which the next targets are formed. When this occurs, the local model remains accurate and the step size continues to grow, which in turn accelerates the divergence of the Q-values. However, this divergence is not solely observed for adaptive methods. Tuned-512, which uses an identical batch size, exhibits this same behavior at a slower rate, while Tuned-32 does not \todo{I think this may be because i didn't check low enough values for my grid search for tuned-512, running a few more values currently}. This suggests the implicit regularization afforded by smaller, noisier batches may help prevent aggresively fitting to inflated bootstrapped targets. This result is consistent with \tocite{Small batch deep reinforcement learning}, which also found smaller batch sizes to more effective with DQN. The divergence exhibited by some globalization strategies in this environment may therefore not be inherent to these methods but rather an unfortunate consequence of their present incompatibility with smaller batch sizes, which we explore further in % todo: ref to section with small vs large batch size
Additionally, we did not re-tune any of DQN's other hyperparameters besides the step size when using larger batch size, which may exacerbate instabilities, as DQN is notoriously sensitive to its hyperparameters \todo{Citation}. 

GRASS may be less afflicted by the feedback loop described above because its chosen step size stabilizes much earlier in the run as compared to the other adaptive methods. Because $\stepSize$ scales a unit vector rather than the raw update with standard trust region methods, the displacement per update is $\stepSize$ rather than $\stepSize \norm{\updateAt{k}}$. Starting from the same $\trustRegionInit$ of $1e-5$, we observe an initial growth period for both GRASS and the trust-region methods. However, for GRASS median gain-ratio quickly falls below $T_{\text{hi}}$ and the step size then shrinks accordingly. Since the effective displacement is smaller by a factor of $\norm{\updateAt{k}}$ for trust region methods, their step size continues to grow. Similarly, unlike GRASS, the examined line search methods adjust the step size each update and always consider the maximum step of $1$, making them more prone to the same divergent behavior. \todo{Q for Adam: not sure if this makes sense re regular TR being more susceptible to divergence bc more updates with larger ss early on} 
...
Unlike in Battle Zone, examining Figure ?a \todo{add fig of step size and sharpness for qbert}% todo: add ref to env after i add 
we observe that for Qbert, step sizes are roughly $10^2$ to $10^4$ times smaller than the tuned baselines, and roughly 10 to 100 times smaller than the chosen step sizes in the other Atari environments (see Appendix %todo: ref appendix with grid for DQN results
). All adaptive methods perform poorly in this environment, and in Figure ?b % add ref to plot
we see that performance is correlated with sharpness in the loss, with GRASS and both line search methods reaching preconditioned sharpness roughly 1000 times greater than the baselines. 
Step size and sharpness are inherently tied, as the EOS literature has shown that in non-convex optimization, parameters settle at a sharpness inversely proportional to the step size. While the adaptive methods operate in sharper regions of parameter space than the tuned baselines in all five environments, the pattern is most extreme in Qbert, where they also perform the worst overall. This finding is aligned with the failure mode reported for line search methods in full-batch supervised learning \cite{roulet2024stepping}, where they selected overly conservative step sizes and failed to reach the EOS. Additionally, we observe sharpness for the baseline methods in all Atari environments does appear to converge to roughly $2/\eta$, as observed for RMSProp in supervised learning \tocite{citation}. Yet, interestingly, these methods utilize the Adam optimizer, for which convergence to $38/\eta$ is expected \tocite{citation}. 
...
We now examine adaptive step size for PPO, beginning with the classic control suite. Similarly to DQN, Figure \todo{fig of return for the three envs} % todo: ref to fig once i put it in
...
shows that chosen step sizes for line search methods and the GRASS variants lie roughly within an order of magnitude above the tuned step size, while those of standard trust region methods are higher as they normalize the update vector. We show plots of return and step size in all environments in Appendix % todo: ref

Moving to Brax, we find that GRASS narrowly outperforms all other adaptive methods and tuned baselines in terms of mean normalized performance over the whole suite, as shown in figure \todo{bar plot of normalized ppo returns} % todo: ref to fig once i put it in
...
As with DQN, we find that GRASS and line search methods fall within an order of magnitude of the baseline while the Trust Region methods have larger step sizes. Notably, no methods exhibit the divergent nor overly conservative step sizes observed in some Atari environments with DQN. This pattern holds in all environments across the Brax suite as shown in Appendix \todo{ref appendix}. 
We hypothesize this is primarily due to using PPO rather than a product of the Brax environments, for several reasons. First, unlike the single-step Q-learning loss used by DQN, the critic targets in PPO are computed using $\lambda$-returns, which place most of their weight on observed rewards rather than current value estimates and are frozen for the duration of each update phase. Moreover, the critic target involves no maximization over actions, so it does not contain the inherent upward bias that can result in overestimation with DQN \todo{cite double q paper}. PPO is also on-policy, implying that states with inflated value estimates will actually be visited under the learned policy and their estimates subsequently corrected. These properties mitigate the feedback loop described in Section~\ref{sec:dqn_results}. Additonally, the clipped surrogate objective bounds the actual decrease in the policy loss produced by any individual update. 
...
% note (opus): the first two arguments hold as written. The third did not. Clipping does not bound the policy change or the loss decrease, since ratios can and do leave the clip range, so I restated it as saturation. This version also acts on the step size criteria rather than the update, which is what you want here. It covers the actor only, as the critic loss in Algorithm~\ref{alg:ppo} is unclipped.
% note (opus): the cleanest way to separate algorithm from environment would be PPO on the Atari-5, even a partial run.

The superior performance of GRASS relative to the Trust Region methods in this setting suggests that preserving the magnitude of the update, rather than using only its direction, is especially useful in these environments. While the displacement in parameter space for GRASS is proportional to the magnitude of the raw update vector, traditional trust region methods are not affected by this quantity and can only adjust the displacement uniformly for all updates through adjusting $\Delta$.

Lastly, the slightly stronger performance of PALS compared to regular backtracking may be attributed to selecting more conservative step sizes in the RL control setting, where stochastic estimates of non-stationary objectives can be high-variance. Backtracking accepts the largest candidate satisfying the Armijo condition, while PALS selects the candidate that provides the greatest reduction in $f_k$ among those that satisfy the decrease condition. This notion is supported by the consistently smaller critic step sizes for PALS relative to Armijo in Humanoid, as shown in Figure % todo: ref fig
This pattern also holds across the Brax suite, with step size for all environment shown in % todo: ref to the appendix 
...
shows that with small batch size in Name This Game, the accepted step sizes are roughly 1000 times larger than the tuned baseline, and performance flatlines. This pattern holds in all environments, as shown in Appendix % todo: ref appendix
At the larger batch size, step sizes are much closer to the fixed baseline, though GRASS appears to select values roughly 100 times smaller than the other adaptive methods. Examining the return in figure % todo ref to fig
...
This stark difference in both step size and performance when varying batch size may result from using the same batch to both to produce an update vector and to measure its performance. With a batch of only 32 transitions, the minimizer of $f_k$ can lie far from the minimizer of $f$, so a step that is too large for the full objective may still produce a large decrease on the batch loss. As a result, the Armijo condition and the gain-ratio are satisfied for step sizes that are far too large. 

Figure % todo: ref fig
shows an approximation of the loss landscape in Name This Game for a typical update with PALS using small and large batch size. We construct this approximation using the evaluated loss at the displacement from the current parameters % todo: latex math for this. 
for each candidate step size in the line search. We compare the actual loss against the acceptance threshold given by the line % todo: math for the line consistent with background section for LS
We find that the loss on the small batch is smoother than that of the large batch, allowing for a larger displacement in the update even though the smaller batch loss may not be reflective of the true loss. % todo: say this in a more concise and clear way and split into two sentences
Increasing the batch size reduces the variance of $f_k$, and therefore a decrease measured on an individual batch is a more reliable indicator of progress on the full objective. We observe the accepted step size is tempered accordingly. 
...
% PPO
Turning to PPO in Brax, we again find that all adaptive methods perform much better with large batch size, as shown in Figure % tood: ref fig
Figure % todo: ref fig
shows actor and critic step sizes in Humanoid, where we observe that actor step sizes are much larger at batch size 64 for the line search methods (flatlining at 1, the meximum value in the search) and basic trust region. By contrast, actor step size differs by less than an order of magnitude between batch size 64 and 1024 for GRASS and the Cauchy point method. Critic step size is relatively consistent between the two batch sizes for all adaptive methods. The poor performance associated with an actor step size of 1 is unsurprising in these high-dimensional control environments. More notably, the large differences in performance for GRASS and the Cauchy Point for relatively small changes in actor step size reflect the sensitivity of PPO to the actor step size in these environments.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/ablations/2_batch_size/batch_ppo_mujoco_step__opt=adam.png}
    \caption{Effect of batch size on PPO with Adam on Brax.}
...
from step size selection when applying globalization strategies with Adam. When using the Adam update $u_k$ to select step size, we handle ascent directions by defaulting to the smallest candidate step size $\stepSizeInit \beta^B$ with linesearch and using $\rho=0.5$ for the corresponding step in the median gain-ratio with trust region methods, though we find this rarely occurs in practice. Figure % todo: ref fig side a
compares normalized performance in the Atari-5 for GRASS and PALS with and without using momentum in step size selection.
\begin{figure}[htbp]
    \centering
...
\end{figure}
We observe that both methods fail entirely with momentum in step selection. Examining Figure % todo ref fig side b
we see that the chosen step sizes are roughly $10^5$ times larger in Name This Game when step size selection is performed directly with $u_k$. Figure  \todo{plot of loss along update direction}% todo: ref to fig 
Interestingly, this is the opposite effect of what has been observed when combining globalization strategies and momentum-based optimizers in supervised learning, where frequent ascent directions result all step sizes being rejected \tocite{paper that does ls with amsgrad}. 

Investigating further, Figure \todo{plot of loss along update direction}% todo: ref to fig 
shows an approximation of the loss landscape in Name This Game for a typical update when using both the momentum and no-momentum selection strategies with PALS. Similar to the finding for small batch size, when using $u_k$ in the search, the actual loss matches the first-order model over a much larger distance in parameter space than for $\hat{u}_k$, allowing for larger accepted step size. When removing momentum from the search direciton, the curvature is shaper, resulting in more conservative steps. \todo{Q for Adam: have spent too much time trying to figure so tabling for now, but stil not clear to me why this is the case, any thoughts?}
...
\begin{figure}[htbp]
    ...
    \caption{Adam direction ablation for PPO on Brax.}
    \label{fig:adamdir_ppo_mujoco}
\end{figure}
In Figure % todo: ref fig b
we see the actor step sizes for the line search are much larger than the tuned baseline when using $u_k$, while they are within an order of magnitude with $\hat{u}_k$. For GRASS and the trust region methods, the actor step sizes are consistent between the two variants, but the critic step size is larger when using momentum in selection. Figure % todo: ref fig
shows approximations of the actor and critc losses for PALS in Humanoid. Similarly to DQN, we observe the larger critic step sizes are a product of smoother loss along the momentum direction, though the actor appears unaffected. \todo{add comment about why this is, but still not sure as above with DQN}. 
...
Interestingly, this effect does not hold for PPO in Brax, where trust-region-based methods appear compatible with the single-step gain ratio. Figure % todo: ref figure
...
\begin{figure}[htbp]
    ...
    \caption{Effect of gain-ratio window on PPO with Adam on Brax: normalized performance and step size.}
...
\end{figure}
Figure % todo: ref fig
compares the performance of PALS and GRASS in Qbert with and without SAM, an environment both methods previously failed to learn. When using SAM, the adaptive methods are competitive with the tuned baselines, whose performance is also slightly improved. Figure % todo: ref fig
...
To address this issue with GRASS, one can increase the growth threshold $T_{\text{hi}}$ in the gain-ratio
...
\begin{figure}[htbp]
    ...
    \caption[Adaptive step size methods with Adam for DQN in Classic Control.]{Adaptive step size methods with Adam for DQN in Classic Control. Return curves averaged over 20 seeds.}
...
\begin{figure}[htbp]
    ...
    \caption{PPO with Adam on Brax: return and step size over learning.}
