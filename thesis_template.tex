...
In the tabular setting, if $\dynamics$ and $\rewardFunction$ are known, this objective can be minimized by iteratively solving the system of Bellman equations in % todo: ref bellman eq from above
via dynamic programming \cite{sutton1998introduction}.\todo{should I formally introduce DP? My initial intention was to avoid this for brevity but I ended up going in depth in other areas anyways}. When $\dynamics$ and $\rewardFunction$ are unknown, policy evaluation becomes an online stochastic optimization problem \todo{citation}, as algorithms must construct estimates of $\vfn$ and $\qfn$ from stochastic trajectories and then minimize the MSE. Once the update targets are obtained, any optimization algorithm may be employed to reduce the loss, however all pseudocode presented in the the next two sections uses stochastic gradient descent (SGD). 
...
\begin{equation}
    f_k(\xAt{k} - \stepSizeAt{k} A_k^{-1} \nabla f_k(\xAt{k})) \le f_k(\xAt{k}) - c \stepSizeAt{k} \norm{\nabla f_k(\xAt{k})}^2_{A_k^{-1}},
    \label{eq:adam_armijo_condition}
\end{equation}
while still updating with $u_k = -A_k^{-1} m_k$. This is also the apporach used for linesearch with AMSGrad in \tocite{the paper that did this with amsgrad}. 
...
\end{aligned}
\end{equation}
The step size $\stepSizeAt{k}$ is adapted via the gain ratio computed from $s_k$, following Algorithm~\ref{alg:grass}, and the Cauchy point scaling is $\tau_k = 1$ for GRASS and the first-order trust region method, or is obtained by applying Equation~\eqref{eq:cauchy_tau} to $s_k$ in place of the gradient % todo: should this not be the squared gradient since s_k is the inner product of the grad and the update vec? confirm if this is correct
for the standard Cauchy point method. The parameter update is then taken along the momentum direction:
\begin{equation}
...
The maximum eigenvalue measures the curvature of the objective along the direction in which it is most strongly curved, so a large $\lambda_{\max}$ means the loss rises steeply for a small displacement in at least one direction. The definition changes when applying the peconditioner % todo: math for the preconditioner
to the gradient, as this is equiveelnt to optimizing the preconditioned objective % opus todo: latex math for preconditioned objective? % opus todo: make this a bit more specific and clean. Not sure if my interpretation is correct. Be concise and edit this sentence and add 1 setence max if you need to. 
...
Though we do not use SAM unless noted explicitly, when combing SAM with globalization strategies as described in Section~\ref{sec:sam}, we set $\kappa$ to the recommended default value of $0.05$ \tocite{SAM paper}. We also utilize layer norm in tandem with SAM, following previous applications of SAM in RL \tocite{atari sam paper}.
...
In this chapter, we examine the performance of all the examined adaptive methods with DQN and PPO. For DQN, we identify all adaptive methods perform well in the classic control suite, demonstrating that globalization strategies can serve as an effective means of automatic step size selection in deep RL. Moving to more complex environments, we find that GRASS greatly outperforms the other adaptive methods with DQN in Atari. However, GRASS still struggles relative to the tuned baseline with smaller batch size, as globalization stategies appear to exhibit several distinct failure modes in this setting. With PPO, we find that all methods perform similarly well in classic control, while GRASS is the best performing method when used with PPO in Mujoco.
...
shows the chosen step sizes for all methods in the Acrobot environment. For the Cauchy point method we report $\tau_k \Delta_k$, as this represents the actual step size for the update. At the beginning of learning, step sizes for GRASS and the trust region methods quickly rise from their starting point $\eta_0$ to a reasonable value for the environment. We observe the chosen step sizes for GRASS and the line search methods all lie within an order of magnitude below the fixed step size baseline. Step sizes for standard Trust Region and the Cauchy point method are much larger, as the effective step size for these methods is actually $\Delta_k / \norm{\updateAt{k}}$ and $\tau_k \Delta_k / \norm{\updateAt{k}}$, respectively. \todo{Q For Adam: If we wanted to make make everything on the same scale we could report norm of the displacement in weight space for each method instead, thoughts? We could also plot the technical step size applied to the raw update which would have a 1/norm(u) term but i did not log the unnormalized norm(u) so would have to rerun a bunch of stuff :/ } The larger step size for regular armijo relative to PALS reflects the bias of backtracking methods twoards larger step sizes, even when smaller ones may achieve equal or greater reductions in the objective. Plots of return and step size for all environments are given in appendix % todo: ref appendix
...
we observe these collapses in performance are correlated with steadily increasing step size, loss, gradient norms, and Q-values. This finding suggests globalization strategies may be prone accelerating the effects of the deadly triad. When a step that reduces the current TD loss also inflates the Q-values from which the next targets are formed, the gradient norms continue to grow as the Q-values diverge, therefore the gain-ratio remains high and the Armijo condition is consistently satisfied, allowing for larger step sizes. % opus todo: make not a run on and confirm that this makes sense
Note this divergence is not exclusive to adaptive methods. Tuned-512, which uses the same batch size as the adaptive methods, exhibits the same behavior at a slower rate, while Tuned-32 does not. This suggests the instability is driven by the combination of large steps and low-noise, large-batch updates, rather than being inherent to the adaptive methods. % opus todo: confirm this makes sense. not really sure why it would be worse with lower noise updates
GRASS may not be affected by this issue because it is the only method where the chosen step size stabalizes early in the run. For the trust region methods, because $\eta$ scales a unit vector rather than the raw update, moving a given distance in parameter space requires step size that is $\norm{\updateAt{k}}$ times larger. Starting from the same $\eta_0$, standard trust region methods must therefore grow the step size over many more windows before the gain-ratio is too small to warrant an increase, allowing for a longer period in which divergence may begin take place % opus todo: this doesnt make sense to me because arent steps smaller during this period? not sure what make sense. dont let my concern bias your assesment though
Interestingly, this phenomenon is not observed in Name This Game. As shown in % todo: ref appendix with dqn atari curves
we again observe decline in return correlated with increasing step sizes, however there is no corresponding trend in the loss, TD-error, gradient norms or Q-values, all of which are indistinguishable from the tuned baselines. However, the decline does appear to once again be a consequence of the larger batch size. Recall that Tuned-32 and Tuned-512 use an identical step size of $6.25\mathrm{e}{-5}$, and we observe that Tuned-512 performs much worse than Tuned-32 in this environment. Over the course of the run, Armijo and PALS raise their step sizes from roughly $5\mathrm{e}{-6}$ to exactly this value, and their return decays from a peak above Tuned-32 down to the level of Tuned-512. Rather than failing by diverging, these methods gradually settle on a step size that clearly performs poorly at their batch size, and performance declines accordingly. By constrast, GRASS, which selects step sizes roughly three orders of magnitude smaller, achieves the best performance in this environment. 
Once agin, this failure mode appears to be a consquence of the dependence of globalization methods on large batch size in RL control examined in Section~\ref{sec:batch_size} in tandem with the DQN algorithms sensitivity to batch size. Note that we did not re-tune any of the DQN algorithm's other hyperparameters besides learning rate when increasing the batch size. This potentially explains both the instabilities observed in Battle Zone and Name This Game, as DQN is notoriously sensitive to its hyperparmeters \todo{Citation, Q for Adam: any recs or can i just say this?}. % opus todo: i realized that the lowest ss i tested for large batch size was the one selected. I think that in reality the issue in name this game is that step size just goes too large for the batch size, not that anything in particular is going wrong, its just that the ratio of the actual to predicted decreaes allows for larger step size for LS and trust region when in reality that is just not good. Please write a commented version that explains this for this case. Also for both versions (the one in the text and the commented one, I am not sure why once again grass does better). Note that for the explaination where the tuned ss is actually just lower for large batch, i think the explaination is probably the similar to that of the explatioation for why large batch size is good in sec:batch_size, but let me know what you think.  

By contrast, while all adaptive methods perform particularly poorly in Qbert, examining Figure ?a \todo{add fig of step size and sharpness for qbert}% todo: add ref to env after i add 
we observe that step sizes are roughly 2-4 orders of magnitude smaller than the tuned baselines, and roughly 1-2 orders of magnitude smaller than the chosen step sizes in the other Atari environments (see Appendix %todo: ref appendix with grid for DQN results
), with the exception of the divergent curves for Battle Zone. In Figure ?b % \todo{add fig of step size and sharpness for qbert}% todo: add ref to env after i add 
...
Step size and sharpness are inherently tied, as the EOS finding showed that with non-convex objective, optimizers settle at a sharpness inversely proportional to the step size. While the adaptive methods operate in sharper regions of parameter space than the tuned baselines in all five environments, the pattern is most extreme in Qbert, where they also perform worst. This finding is alligned with the failure mode reported for line search in full-batch supervised learning \cite{roulet2024stepping}. Additionally, we observe sharpness for the baseline methods in all Atari environments does appear to converge to roughly $2/\eta$, as observed for RMSProp in supervised learning \tocite{citation}. Yet, interestingly, these methods utilize the Adam optimizer, for which convergence to $38/\eta$ is expected in SL \tocite{citation}.

These results suggest that several unique failure modes exist across the examined adaptive methods, which may result in both overly conservative and aggressive step sizes depending on the environment. Nevertheless, the strong performance of GRASS relative to Tuned-512 and competitve performance with Tuned-32 in $3/5$ environments suggests that it remains a promissing approach for adaptive step size with DQN. 
...
Moving to the Brax MuJoCo, we find that GRASS narrowly outperforms all other adaptive methods and tuned baselines in terms of mean normalized performance over the whole suite, as shown in figure \todo{bar plot of normalized ppo returns} % todo: ref to fig once i put it in
...
we see that GRASS and PALS are consistently competitive with the best tuned baseline in $4/5$ environments, while the backtracking Armijo and Trust Region methods can sometimes perform much worse than fixed-schedule baselines. Additionally, Tuned-64 achieves higher return than Tuned-1024 in $4/5$ environments, yet reaches less than a third of its return in Humanoid. No fixed pairing of step size and batch size is therefore reliable across the whole suite. Note that we never observe a failure of this magnitude for GRASS or PALS, demonstrating the benefit of adaptive step size selection in this setting is primarily reliability across environments rather than a large gain in any individual environment.

Figure \todo{plot of step size for ant} % todo: ref to fig once i put it in
shows the actor and critic step sizes in Ant.
Once again, we find that GRASS and line search methods fall within an order of magnitude of the baseline while the Trust Region methods have larger step sizes. Noteably, no methods exhibit the divergent nor overly conservative step sizes observed in some Atari environments with DQN. This pattern holds in all environments across the Brax suite as shown in Appendix \todo{ref appendix}. 
We hypothesize this is primarily due to using PPO rather than a product of the Brax the environments for several reasons. First, unlike the single step Q-learning loss used by DQN, the critic targets in PPO are computed using $\lambda$-returns, which are less dependent on current value estimates, preventing inflation when the targets are reset after each data collecton phase. Second, PPO is an on-policy algorithm, implying that actions with inflated value estimates from the critic will actually be sampled by the learned policy % todo: latex math for this consistent with the background section
and thus their value estimates will subsequently be tempered. Third, the clipped surrogate objective bounds the actual decrease in the policy loss produced by any individual update. All three of these properties mitigate the feedback loop described in Section~\ref{sec:dqn_results}. % todo: is it fair to characterize that section as describing a feedback loop? is that made clear enough in that section? 
Additonally, the comparatievly lower-dimensional proprioceptive observations in Brax likely pose a better conditioned optimization problem than learing from raw pixels.
% opus todo: do all my explainations re why ppo doesnt have the issues dqn does given above make sense? 

The superior performance of GRASS relative to the Trust Region methods in this setting suggests that preserving the magnitude of the update, rather than using only its direction, is especially useful in these environments. While the dispalcement in parameter space for GRASS is proportional to the magnitude of the raw update vector, traditional trust region methods are not affected by this quantity and can only adjust the displacement uniformily for all updates through adjusting $\Delta$.
...
\chapter{Challenges in Complex Environments}
...
The first three sections examine design choices already employed in the experiments of the previous chapter. The use of large batch size, the strategy for handling momentum, and the median gain-ratio were all used by default in every experiment reported so far. We now remove each in turn to exmaine their effect on learning, which in all three cases is characterized by overly aggressive step sizes and poor performance. The final section is an addition rather than an ablation. We return to the finding that globalization strategies can select overly conservative step sizes in settings with sharp loss landscapes, and examine Sharpness Aware Minimization as a potential remedy.

\section{Sensitivity to Batch Size} \label{sec:batch_size}
% DQN
To examine the effect of batch size on the performance of globalization strategies in RL, Figure ?a% todo: ref to figure
compares the normalized performance of GRASS and PALS averaged across the Atari-5 with standard DQN batch size of 32 and the larger batch size of 512 utilized and the preceding experiments. Similar to what has been observed in SL, a larger batch size massively improves the control performance of both methods \cite{roulet2023interplay}, with both GRASS and PALS completely failing to learn with small batch size. Furthermore, figure % todo: ref fig b
shows that with small batch size in Name This Game, the accepted step sizes are roughly 3 orders of magnitude larger than the tuned baseline, and performance flatlines. This pattern holds in all environments, as shown in Appendix % todo: ref appendix
At the larger batch size, step sizes are much closer to the fixed baseline, though GRASS appears to select values roughly two orders of magnitude smaller that the other adaptive methods.
Examining the return in figure % todo ref to fig
, we find that despite the improved performance with large batch size, PALS still suffers from performance degradation roughly a third of the way though the run, while GRASS outperforms both tuned baselines. 

This stark difference in step size and resulting performance may result from the fact that the same batch is used both to produce an update vector and to measure its performance. With a batch of only 32 transitions, a step that is too large for the full objective may still produce a large decrease on the batch loss, as smaller batches should be much easier to fit. As a result, the Armijo condition and the gain-ratio are satisfied for step sizes that are orders of magnitude too large. This is exact situation ruled out by the interpolation condition in Equation~\eqref{eq:interpolation}, which may not hold in deep RL, where the regression targets are non-stationary and a large replay buffer is unlikely to be fit exactly. Increasing the batch size reduces the variance of $f_k$, and therefore a decrease measured on an individual batch is a more reliable indicator of progress on the full objective. We observe the accepted step sizes are tempered accordingly.
\todo{ Plot comparing loss landscape for small vs large batch size}
...
% PPO
Turning to PPO in Brax, we find that once again all adaptive methods perform much better with large batch size, as shown in Figure % tood: ref fig
Figure % todo: ref fig
shows actor and critic step sizes in Humanoid, where we observe that actor step sizes are much larger at batch size 64 for the line search methods (flatlining at their maximum value of 1) and basic trust region. By contrast, actor step size differs by less than an order of magnitude between batch size 64 and 1024 for GRASS and the Cauchy point method. Additionally, critic step size is relatively consistent between the two batch sizes for all adaptive methods. The poor performance associated with an actor step size of 1 is unsurprising in these high-dimensional control environments. More notably, the large differences in performance for GRASS and the Cauchy Point for relatively small changes in actor step size reflect the sensitivity of PPO to the actor step size in these environments.
\begin{figure}[htbp]
    \centering
...
We now examine the effect of removing the momentum term %todo: latex for momentum term consistent with the momentum section in the methods
from step size selection when applying globalization strategies with Adam. Figure % todo: ref fig side a
compares normalized performance in the Atari-5 for GRASS and PALS with and without this aproach. 
\begin{figure}[htbp]
    \centering
...
\end{figure}
We observe that both methods fail entirely when applied naively on the Adam update vector. Examining Figure % todo ref fig side b
we see that the chosen step sizes are roughly five orders of magnitude larger when step size selection is performed directly on the Adam direction with momentum % todo: math for this instead of writing it out (see no momentum mehtod section for what to put)
and both methods fail to learn. Interestingly, this is the opposite effect of what has been observed when combining globalization strategies and momentum-based optimizers in supervised learning, where frequent ascent directions result all step sizes being rejected \tocite{paper that does ls with amsgrad}. The inflated step size is not simply due to a difference in the magnitude of the update vector, as Figure \todo{add fig of adam update norm and slope for naive vs non-naive} % todo: ref fig
shows the update norms are closely aligned with and without momentum. Examining the inner product of the gradient and the update % todo: latex for this consistent with the adaptive step size background and methods
, which we refer to as slope, we observe the Adam updates % todo: latex math for this
have roughly an order of magnitude larger slope as compared to when momentum is removed % todo: latex math for this update
. Because the slope effectively controls how conservative both methods are in growing the step size, we hypothesize the momentum direction being misalligned with the gradient % todo: latex math for momentum direction and gradeint on current update
actually allows for larger accepted step size due to the relaxed predicted decrease from the model in the gain-ratio and the sufficient decrease condition. However, for the observed difference to arise, it must hold that % todo: latex math for adam direction curvature being less than or equal to rmsprop direciton, can we just use second directional derivative here? i think that makes sense?
To examine this hypothesis, Figure  \todo{plot of loss along update direction}% todo: ref to fig 
shows an approximation of the loss landscape on several individual batches sampled at different points in learning with PALS when using %todo: latex for adam update without momentum and adam udpate with momentum
to select step size. We construct these approximations using the evaluated loss for each candidate step size in the line search. % move this descrption up to the small vs large batch section where we first introduce this plot
We find that

Turning PPO in Brax, we observe that performance is also greatly improved for all methods when using % todo: latex math no momentum direction 
in place of % todo: latex math for momentum direction
for selecting step size, as shown in Figure % todo: ref fig a 
. 
\begin{figure}[htbp]
    \centering
...
\end{figure}
In Figure \todo{get env besides humanoid to demonstrate this} % todo: ref fig b
we see the actor step sizes for the adaptive methods are much larger than the tuned baseline when using % todo: math for regular update with momentum
while they are within an order of magnitude with  % todo: math for regular update without momentum
\todo{will need to change metric from LR to displacement to show this better for all methods, currently claim for LR incorrect. see note above re LR vs displacement}. We hypothesize this may occur for the same reasons described above with DQN, as Figure
\todo{add fig of adam update norm and slope for naive vs non-naive} % todo: ref fig when i put it in
shows that once again update norms are consistent between % todo: latex math for no momentum and momentum direction 
while slope differs by \todo{complete}. \todo{plot of line search values along update direction}

\section{Comparing the Median and Single-Step Gain-Ratio} \label{app_gain_ratio_ablation}
We compare the traditional single-step gain-ratio (Equation~\eqref{eq:tr_ratio}) against the median gain-ratio over a window of $K$ updates introduced in GRASS (Algorithm~\ref{alg:grass}), which we applied with all trust-region-based methods in all experiments thus far. Figure % todo: ref fig
examines the performance of these two approaches across the Atari-5 for GRASS. 
\begin{figure}[htbp]
    \centering
...
\end{figure}
\todo{note for Adam: didn't run other methods to save compute, seems like bad practice so maybe run regular TR too? note i run them all for PPO mujoco bc its cheaper} Note that a gain-ratio window of $1$ corresponds to updating the step size after every update using the single-step ratio, while the larger window matches the $K$ used in the main experiments. We observe that performance is much improved with $K$ set to 500, which corresponds to $1/4$ of the target network update interval, suggesting improved robustness to noisey gradient estimates in determining the true accuracy of the first-order model. Examining the step sizes for Name this Game in % todo: ref fig
we see the single-step ratio results in larger accepted step size and worse performance, similar to the effect of small batch size. \todo{add return curve to this for name this game}. 

Interestingly, this effect does not hold for PPO in Brax, where trust-region-based methods appear compatable with the single-step gain ratio. Figure % todo: ref figure
shows that performance in Brax is relatively unchanged when using the single-step ratio vs a ratio of $5$, which corresponds to $1/4$ of the updates per data collection phase when using batch size 1024, though GRASS is slightly improved. 
\begin{figure}[htbp]
    \centering
...
\end{figure}
\todo{note for adam: this is a much smaller difference then in atari but ive ablated this a long time ago and found that using a much larger window still didnt make a difference}. We also observe that step sizes remain largely unaffected by this change, as demonstrated for Humanoid in Figure % todo: ref fig. 
The lesser importance of the aggregated gain ratio with PPO may be because batches are drawn from an approximately on-policy dataset of 2048 samples, while with DQN they are sampled from a replay buffer of size $1,000,000$, potentially resulting in higher variance in the Gain-Ratio between batches. % opus todo: not sure if this tracks, can you check if this makes sense and let me know what you think?
The decreased impact between the two treatments for PPO is also likely influenced by the much smaller difference in $K$ as compared to DQN. 

\section{Mitigating Sharp Minima with SAM}
We now return to the finding in section % todo: ref section
that globalization strategies can select overly conservative step sizes in some settings, moving into sharp regions of parameter space. We introduce a strategy for resolving this issue using Sharpness Aware Minimization in tandem with globalization strategies as described in section % todo: ref section
\begin{figure}[htbp]
    \centering
...
\end{figure}
Figure % todo: ref fig
compares the performance of PALS and GRASS in Qbert with and without SAM, an environment both methods previously failed to learn. When using SAM, the adaptive methods are competitive with the tuned baselines, whose performance is also slighly improved. Figure % todo: ref fig
shows that when using SAM the chosen step sizes are much larger, within an order of magnitude below or above the tuned baseline. As a result, we see that sharpness is also decreased and now closely matches that of Tuned-32 for both the adaptive methods and Tuned-512. 
However, the steady increase in step size for both methods is potentially concerning, as it resembles the pattern of the failure modes observed in %todo: ref dqn performance section
where some methods suffered from performance degradation over the course of the run. 
Indeed, examining Figure % todo: ref fig
we see that while performance across the Atari-5 is improved for PALS and Tuned-512, performance for GRASS is unchanged due to worsened behavior in some environments, including Battle Zone, as shown in Figure % todo: ref fig
. 
\begin{figure}[htbp]
    \centering
...
\end{figure}
In this environment we observe the steadily increasing step size and corresponding performance degradation as identified for other globalization strategies without SAM in % todo: ref section discussing DQN performacne
\todo{look at stats with sam to try to figure out why this is}
To address this issue with GRASS, one can increase the growth threshold in the gain-ratio % todo: latex math for this from methods section
as shown in figure % todo: ref fig
which improves performance by making the method more conservative. However, we found that this change to be overfit to this environment suite, harming performance in classic control \todo{Adam: is it ok to just say this and not provide graphs?}. 
Performance for tuned-512 is also greatly improved with SAM, suggesting the loss of the implicit regularization afforded by small batch size also hurts the performance of this method. Conversely, tuned-32 fails with SAM, \todo{Note for Adam: Think this is just because I didn't retune with SAM, would need to resweep SS in the atari-5 with SAM for both methods, not sure what to do here, should we run or just ablate for adaptive methods? The latter seems not great practice.}
