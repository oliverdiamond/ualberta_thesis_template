...
\begin{algorithm}[h] % Local todo: make consistent with terminology and notation and formatting used in sec:adaptive
...
Several apporaches have been proposed for combining line search methods with momentum in supervised learning \tocite{citations}, however there remains no clear consensus regarding this challenge in the literature. Additonally, to the best of our knowledge, this issue has not been examined in RL nor with trust region methods \todo{confirm this}. This issue is especially important in deep RL, where in recent years the Adam optimzier, which uses a bias-corrected form of Polyak momentum, has become ubiquitous \tocite{citation}. For combining globalization strategies with the Adam in RL, we adopt the strategy used for combining line search with Polyak momentum in \cite{vaswani2019painless}, whereby momentum is not used in the search direction but still used in the applied update. Similarly, for trust region methods we do not use momentum when computing the gain-ratio or $\tau$ for the Cauchy point \todo{make sure tau is consistent with cauchy psuedocode} however momentum is still used in the applied update. 
...
This phenomenon has also been characterized in reinforcement learning, 
% local todo: Clear and concise description of basic sharpness aware minimization (SAM) with basic pseudocode and explaining how it gets used with the globalization strategies, which can be seen by examining SAM usage in /Users/oliverdiamond/Desktop/alberta/research/linesearch_brax/src/train/ppo/ppo_continuous_action.py and src/train/dqn/dqn_control_atari_ckp.py. Basically we just always evaluate the SAM loss instead of the regular loss. Don't use rho for the radius since we use rho elsewhere.
\todo{cite the paper that used SAM and layer norm for continual learning and the Q-SAM paper that uses SAM in offline RL}
Sharpness aware minimizaiton has been used to encourage better generalization in offline RL and for mitigating placisticity loss in online RL, however the effect of loss landscape sharpness on the performance of globalization strategies it has not been studied in the online RL setting 
...
GRASS and all the trust-region-based methods examined in this work share the same hyperparameters: % todo: list hypers from GRASS pseudocode
Following the classical optimization literature, we do not tune % todo: all hypers except the rho buffer size
, holding them constant across all experiments using standard values from the literature \cite{nocedal2006numerical}. For % todo: value for the size of the rho buffer
For the line search methods, we use 30 backtracking steps, starting the search at 1 with exponential decay hold all hyperparamers constant across all experiments, with the exception of 
Line searche methods always started from $\stepSizeInit = 1$ and performed $B=30$ backtracking steps with $\beta=0.7$, except . We set $c=0.1$ for Armijo variants \cite{vaswani2019painless}
When using SAM as described in  % local todo: ref to section
we do not tune % local todo: latex for sam  
...
We use line search for both the parameterized policy and state-value network.
% DQN details
We use adaptive step size methods for learning the parameterized action-value network % todo: latex for this consistent with background
...
As the adaptive step size methods examined in this work employ larger batch sizes than the values typically used by DQN and PPO, we examine fixed step sizes with both standard and larger batch sizes matching those of the adaptive methods. We perform tuning separately for each batch size, and we refer to a tuned fixed step size with a given batch size $N$ as \textit{Tuned N}. Ranges used in the grid search and the selected fixed step size for all optimizers, algorithms and environment suites are given in % local todo: ref to a new appendix section that has a table giving these values. 

\section{Experiments and Performance}
In all experiments, we deploy the agent in the environment by running the standard RL interaction loop described in % local todo ref to relevant background section
