...
\begin{equation}
    ...
    \label{eq:pg}
\end{equation}
Intuitively, moving $\theta$ in this direction increases the probability of actions with high value. One advantage of this approach over value-based methods is that a well-performing policy may be simpler to approximate than its value function. For example, an optimal policy might require only a simple linear decision boundary to separate good actions from bad ones, whereas the value function itself may be highly non-linear. An additional benefit is that directly parameterizing $\pi_\theta$ allows for easily learning policies over pseudo-continuous action spaces, which arises when the set of all possible feature vectors to which the underlying discrete actions are mapped under function approximation is vast and therefore treated as approximately continuous % todo: I want this to describe that its not nessesarily vast but also dense if that makes sense, also this is a bit of a run on as is so probably need two sentences or to make more concise
. Policy-based methods can simply output the parameters of a continuous probability distribution such as the mean and variance of a Gaussian, from which actions can be directly sampled.
...
\chapter{Methods} \label{sec: methods} % todo: title for this section that actually describes what is in it, not sure what to put since we cover 3 distinct things as describe in the para below. 
...
\end{equation}
$\nabla f_k^{\text{SAM}}(x)$ is therefore approximated with $\nabla f_k(x + \hat{\epsilon}(x))$, treating $\hat{\epsilon}(x)$ as a constant with respect to $x$ (i.e., its own gradient is stopped). This approximation is reasonable because % todo: finish this sentence. be concise and not overly verbose
...
While the classic control environments are traditionally defined with continuous action spaces, their action space may also be discretized for use with discrete-action algorithms like Q-learning. Specifically, we examine variants of the Acrobot, CartPole and MountainCar environments implemented as modifications of the corresponding \texttt{gymnax} \tocite{gymnax} environments, with modifications made to bring their dynamics closer to those originally described by \cite{sutton1998introduction}. These modifications are detailed in Appendix~\ref{app_classic_control_diffs}.

The second set of environments are taken from the Arcade Learning Environment (ALE) \tocite{ALE}. Unlike classic control, the ALE presents the agent with raw pixel observations of Atari 2600 games, yielding a much higher-dimensional observation space. This makes learning an accurate value function or policy a substantially harder optimization problem than the low-dimensional classic control settings.
...
For trust-region-based methods, across all experiments, we use standard values from the classical optimization literature \cite{nocedal2006numerical} \tocite{other source that establishes 0.5 as common (see gemini convo)} of $(T_{\text{lo}}, T_{\text{hi}}) = (0.25, 0.75)$ and $(\alpha_{\text{shrink}}, \alpha_{\text{grow}}) = (0.5, 2)$. Though setting $\alpha_{\text{shrink}}$ to $0.25$ is also common, we use $0.5$ as this value allows for less aggressive adaptation, which we believe to be appropriate for the high-variance regime of online RL. We set $\trustRegionInit$, the initial step size for all trust-region-based methods, to $1\mathrm{e}{-5}$ in all experiments. For $K$, we use $\nicefrac{1}{4}$ of the target network refresh interval $N_{\text{target}}$ for DQN and $\nicefrac{1}{4}$ of the total updates performed after each data collection phase for PPO. All the examined trust-region-based methods utilize the median gain-ratio introduced in GRASS, though we present an ablation comparing this approach against the traditional single-step ratio in Section~\ref{sec:gain_ratio_ablation}. We emphasize that none of the hyperparameter values given above were tuned. As the gain-ratio is invariant to the scale of the loss and gradients, as well as the network parameterization, these default values have been used successfully across convex, non-convex, constrained, and stochastic trust region methods \cite{nocedal2006numerical} \tocite{other sources}. 

For all line search methods, we start the search at $1$ to capture a wide range of values and use a sufficient decrease coefficient of $c=0.1$, which is common for stochastic line search in supervised learning \cite{vaswani2019painless}.To reduce computational cost, DQN experiments in Atari are run with $B=10$ backtracking steps with decay factor $\beta=0.2$, while in all other settings we use $B=30$ with decay factor $\beta=0.7$. The decay factors are set such that the minimum candidate step sizes are approximately $1e-7$ and $2e-5$, respectively, which are both well below the tuned default values described later in this section for their respective settings, preventing implicit tuning of the chosen step size for these methods. For both PALS and the Armijo line search, we apply an EMA on the chosen step size as described in Section~\ref{sec: pals}, with decay rate $\zeta = 0.99$. As with the trust region methods, we do not tune the line search hyperparameters to avoid replacing one tuned parameter with another. All hyperparameters for the line search and trust region methods are given in Appendix % todo: ref to new tables in the appendix for line search and trust region methods, with rows for hypers and columns for the Algo/Env pairs. For line search we only need two columns, one for DQN-Atari and one for "All Other Settings", while trust region I want one column for each of the 4 algo-env combos. Please make the two tables. 

When combining SAM with globalization strategies as described in Section~\ref{sec:sam}, we set $\kappa$ to the recommended default value of $0.05$ \tocite{SAM paper}, which we found to achieve the best performance via cross-environment-tuning \tocite{paper for this} over the Atari-5 (swept values are given in Appendix %todo: add sam kappa values of 0.01, 0.05 and 0.1 to a new table giving the values swept for DQN in the Atari-5. The table will have one row and will be the same style as the tables in Fixed Step Size Tuning Range Appendix Section, which is where it should go, and we should rename that section "Grid Search Ranges For Cross Environment Tuning"
). We also utilize layer normalization \tocite{layer norm} in tandem with SAM, following previous applications of SAM in RL \tocite{atari sam paper}. 

We examine all methods with PPO in continuous-action Classic Control and Brax. We select the step size separately for the actor and critic.
...
In all experiments, we deploy the agent in the environment by running the standard RL interaction loop described in Chapter~\ref{sec: background} for a fixed number of timesteps. We use 150 thousand timesteps in classic control, 3 million in Brax \tocite{open ai baselines}, and 50 million in Atari \tocite{marlos paper}. To assess performance for a single run, we use the mean episodic return \tocite{epirical design in RL}, defined as the average return across all episodes, weighted by episode length:
\begin{equation}
    \text{Mean Episodic Return} = \frac{\sum_{i=1}^{M} T_i G^{(i)}}{\sum_{i=1}^{M} T_i},
    \label{eq:weighted_performance}
\end{equation}
where $M$ is the total number of episodes completed in the run, and $T_i$ and $G^{(i)}$ are the length and return of episode $i$, respectively. For each method, we average this quantity over several random seeds used for initializing the agent's learnable parameters ($\theta$ and $\vparams$ for PPO, $\qparams$ for DQN) as well as the environment's starting state. In Atari we use 3 seeds \todo{Edit once i run more seeds}, while for all other environments we use 20 seeds. To measure aggregate performance across an environment suite, we apply Winsorized Min-Max Normalization with $5^{\text{th}}$ and $95^{\text{th}}$ percentile bounds to the performance of all runs within each environment across all examined methods, average within each method to obtain its mean normalized performance on that environment, and finally average across environments to obtain its overall mean normalized performance. We use the $5^{\text{th}}$ and $95^{\text{th}}$ percentiles to prevent outliers from distorting the min-max normalization; this does not alter the set of runs used in our analysis. We report 95\% confidence intervals for this metric using a stratified bootstrap with $5{,}000$ resamples.
...
Another limitation of this work is that popular deep RL benchmarks fail to demonstrate the need for adaptive step size methods for two distinct reasons. First, these environments are fully observable and do not capture the non-stationarity present in the real-world that motivates the need for methods capable of adapting their hyperparameters during learning. % todo: make not a run on and more clear
