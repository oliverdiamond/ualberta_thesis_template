...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/thesis/performance/2_dqn_atari/dqn_atari_return.png}
    \caption[Learning curves for DQN with Adam on the Atari-5.]{Learning curves for DQN with Adam on the Atari-5. Performance was far less uniform across methods and environments than in classic control, with all adaptive methods except GRASS exhibiting either total failure or a collapse in return in Battle Zone.}
    \label{fig:dqn_atari_return}
\end{figure}
...
This finding suggests globalization strategies may be prone to accelerating the effects of the deadly triad. Both the Armijo condition and the gain-ratio only measure whether a step reduced the current objective by a sufficient fraction of the predicted decrease from a local model. Neither method accounts for the fact that larger steps may also inflate the Q-values from which the next targets are formed. When this occurs, the local model remains accurate and the step size continues to grow, which in turn accelerates the divergence of the Q-values. However, this divergence was not unique to the adaptive methods: Tuned-512, which uses an identical batch size, exhibited this same behavior at a slower rate, while Tuned-32 did not \todo{I think this may be because i didn't check low enough values for my grid search for tuned-512, running a few more values currently}. This suggests the implicit regularization afforded by smaller, noisier batches may help prevent aggressively fitting to inflated bootstrapped targets. This result is consistent with \tocite{Small batch deep reinforcement learning}, which also found smaller batch sizes to be more effective with DQN. The divergence exhibited by some globalization strategies in this environment may therefore not be inherent to these methods but rather an unfortunate consequence of their present incompatibility with smaller batch sizes, which we explore further in Section~\ref{sec:batch_size}.
Additionally, we did not re-tune any of DQN's other hyperparameters besides the step size when using the larger batch size, which may exacerbate instabilities, as DQN is notoriously sensitive to its hyperparameters \todo{Citation}.
...
Unlike in Battle Zone, examining Figure~\ref{fig:dqn_atari_qbert_step_sharpness}(a), we observed that for Qbert, step sizes were roughly $10^2$ to $10^4$ times smaller than the tuned baselines, and roughly 10 to 100 times smaller than the chosen step sizes in the other Atari environments (see Appendix Figure~\ref{fig:dqn_atari_adam} for the full comparison across environments). All adaptive methods performed poorly in this environment, and in Figure~\ref{fig:dqn_atari_qbert_step_sharpness}(b) we found that performance was correlated with sharpness in the loss, with GRASS and both line search methods reaching preconditioned sharpness roughly 1000 times greater than the baselines.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/thesis/performance/2_dqn_atari/dqn_atari_qbert_step_sharpness.png}
    \caption[\textbf{(a) Left:} Step size and \textbf{(b) Right:} sharpness for DQN on Qbert.]{\textbf{(a) Left:} Step size and \textbf{(b) Right:} preconditioned sharpness $\lambda_{\max}$ for DQN with Adam on Qbert. All adaptive methods selected step sizes far below the tuned baselines and reached far greater sharpness, resulting in poor performance.}
    \label{fig:dqn_atari_qbert_step_sharpness}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{DQN with Adam on the Atari-5: return, step size, and preconditioned sharpness ($\lambda_{\max}$) during learning.}
...
\begin{figure}[htbp]
    ...
    \caption{PPO with Adam in Brax: return and step size during learning.}
