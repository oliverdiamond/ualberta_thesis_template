...
\begin{algorithmic}[1]
\State \textbf{Input:} (possibly stochastic) loss function $f$, initial point $\xAt{0}$, initial step size $\stepSizeInit$, window size $K$, thresholds $\tau_{\text{lo}} = 0.25$, $\tau_{\text{hi}} = 0.75$, factors $\alpha_{\text{shrink}} = 0.5$, $\alpha_{\text{grow}} = 2$
\State Initialize $\stepSize \gets \stepSizeInit$, buffer $R \gets [\,]$, boundary counter $n \gets 0$, iteration counter $k \gets 0$
\For{each update}
    ...
    \If{$k \bmod K = 0$}
        ...
        \If{$\bar{\rho} < \tau_{\text{lo}}$}
            \State $\stepSize \gets \alpha_{\text{shrink}} \cdot \stepSize$
        \ElsIf{$\bar{\rho} > \tau_{\text{hi}}$ \textbf{and} $n \ge K / 2$}
            \State $\stepSize \gets \alpha_{\text{grow}} \cdot \stepSize$
        \EndIf
...
\begin{equation}
    \tau_k =
    \begin{cases}
        1 & \text{if } \inner{\bar{u}_k}{B_k \bar{u}_k} \le 0 \\[4pt]
        \min\!\left( -\dfrac{\inner{\nabla f_k(\xAt{k})}{\bar{u}_k}}{\stepSizeAt{k} \inner{\bar{u}_k}{B_k \bar{u}_k}},\ 1 \right) & \text{otherwise,}
    \end{cases}
...
The maximum eigenvalue measures the curvature of the objective along the direction in which it is most strongly curved, so a large $\lambda_{\max}$ means the loss rises steeply for a small displacement in at least one direction. The definition changes when applying the peconditioner $A^{-1}$ to the gradient, as this is equiveelnt to optimizing the preconditioned objective % opus todo: latex math for preconditioned objective? % opus todo: make this a bit more specific and clean. Not sure if my interpretation is correct. Be concise and edit this sentence and add 1 setence max if you need to.
...
we see that the chosen step sizes are roughly five orders of magnitude larger when step size selection is performed directly with $u_k$
...
shows the update norms are closely aligned with and without momentum. Examining the inner product of the gradient and the update, $s_k = \inner{\nabla f_k(\xAt{k})}{\updateAt{k}}$, which we refer to as slope, we observe that $u_k$
has roughly an order of magnitude larger slope as compared to$\hat{u}_k$. Because the slope effectively controls how conservative both methods are in growing the step size, we hypothesize the momentum direction being misalligned with the gradient actually allows for larger accepted step size due to the relaxed predicted decrease from the model in the gain-ratio and the sufficient decrease condition. However, for the observed difference to arise, it should hold that the curvature of the loss along the momentum direction is no greater than along the no-momentum direction:
\begin{equation*}
    \inner{\frac{u_k}{\norm{u_k}}}{\nabla^2 f_k(\xAt{k}) \frac{u_k}{\norm{u_k}}} \le \inner{\frac{\hat{u}_k}{\norm{\hat{u}_k}}}{\nabla^2 f_k(\xAt{k}) \frac{\hat{u}_k}{\norm{\hat{u}_k}}},
\end{equation*}
since otherwise % opus todo: not sure if this is actually true or if this is the right way to frame this please help
To examine this hypothesis, Figure  \todo{plot of loss along update direction}% todo: ref to fig 
shows an approximation of the loss landscape on several individual batches sampled at different points in learning with PALS when using $\hat{u}_k$ (no momentum) and $u_k$ (includes momentum)
...
We find that \todo{finish when plot is done}

Turning PPO in Brax, we observe that performance is also greatly improved for all methods when using $\hat{u}_k$
in place of $u_k$
...
we see the actor step sizes for the adaptive methods are much larger than the tuned baseline when using $u_k$
while they are within an order of magnitude with $\hat{u}_k$
...
shows that once again update norms are consistent between $\hat{u}_k$ and $u_k$
...
that globalization strategies can select overly conservative step sizes in some environemnts with DQN, moving into sharp regions of parameter space. We introduce a strategy for resolving this issue using Sharpness Aware Minimization as described in Section~\ref{sec:sam}
\begin{figure}[htbp]
    \centering
...
However, the steady increase in step size for both methods is potentially concerning, as it resembles the pattern of the failure modes observed in Section~\ref{sec:dqn_results}
...
\end{figure}
In this environment we observe the steadily increasing step size and corresponding performance degradation as identified for other globalization strategies without SAM in Section~\ref{sec:dqn_results}.
\todo{look at stats with sam to try to figure out why this is}
To address this issue with GRASS, one can increase the growth threshold $\tau_{\text{hi}}$ in the gain-ratio
...
This appendix contains additional pseudocode for the adaptive step size variants introduced in Chapter~\ref{sec: methods}. Algorithm~\ref{alg:armijo-ema} gives the backtracking Armijo line search with an EMA on the chosen step size (Section~\ref{sec: pals}).
