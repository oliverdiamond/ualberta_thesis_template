...
% todo: do i need to mention the other major condition on the variance? I forget what it is. Is this one of the assumptions? I am talking about the painless stochastic gradient paper Vaswani et al.
...
\end{equation}
$\nabla f_k^{\text{SAM}}(x)$ is therefore approximated with $\nabla f_k(x + \hat{\epsilon}(x))$, treating $\hat{\epsilon}(x)$ as a constant with respect to $x$ (i.e., its own gradient is stopped). This approximation is reasonable because % todo: finish this sentence
