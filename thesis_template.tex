...
\chapter{Ablations in and Challenges Complex Environments}
In this chapter we characterize several challenges that arise when naively applying globalization strategies in complex environments with high dimensional state and action spaces. We first present two ablations which characterize how these methods tend to select overly aggressive step sizes in RL when applied with standard small batch sizes and updates produced by momentum-based optimizers. % todo: make this more clear and concise and not a run on 
We then examine using the per step gain ratio with trust region and GRASS methods, demonstrating that this also leads to step size instability and poor performance. 
Lastly, we return to the finding that globalization strategies can select overly conservative step sizes in settings with sharp loss landscapes, examining SAM as a potential solution. % todo: think of better word that proneness % opus todo: I think its awkward that the first three are presented as ablations since we already use them in the dqn section before this, but then sam isnt used (becuase i think it flows better to not use sam initally and there isnt a clear case for using it.) so its an addition. 
...
At the larger batch size, step sizes are much closer to the fixed baseline, though GRASS appears to select more conservative step sizes approximately two orders of magnitude smaller than the baseline. % todo: make cleaner ie i dont want to repeat "the baseline" twice
...
This stark difference in step size and resulting performance between small and larger batch size suggests that the loss produced by the smaller batch is much smoother / gradient holds over longer distance in parameter space becuase ... which results in choosing much larger steps on high variance estimates of the gradient, with catostrphic consequences. % opus todo: make this clear and concise. I am not sure why loss would be smoother/gradient holds longer (not even sure if that is the right way to describe that) is there a reason? Not sure if i am describing it right. This is related to some of my comments above i think 
...
Figure % todo: ref fig
shows actor and critic step sizes in Humanoid, where we observe that actor step sizes are much larger at batch size 64 for the line search methods (flatlining at their maximum value of 1) and basic trust region. By contrast, actor step size differs by less than an order of magnitude between batch size 62 and 1024 for GRASS, CRASS Cauchy and TR Cauchy, which may explain there relatively strong performance at the smaller batch size compared to the other adaptive methods. 
Additioanlly, critic step size is relatively consistent between the two batch sizes for all adaptive methods. While the poor performance corresponding to large step size of 1 is unsuprising in these high dimesional control environment, small changes in actor learning rate corresponding to large differences in performance for GRASS reflects the sensitivity of PPO to actor step size. % todo: make last sentence not a run on and more clean and clear 
