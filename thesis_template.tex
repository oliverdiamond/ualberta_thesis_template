...
Given an update direction $\update$, line search test a set of candidate step sizes to identify the largest step size such that the new parameters resulting from the update yield a sufficient decrease. % todo: make this more clean and concise
...
In contrast the value based methods, actor-criticiexplicitly parametrizing and learning both a value function and the policy itself

% actor-critic
% Note that this can be thought of as another instance of GPI


% Continuous actions (look at s and b) note that the argmax in 
...
we now turn to the two classes of adaptive step size methods examined in this work: line search and trust region. These two apporaches are known to as globalization strategies, as they were designed to esnure fast convergence to a local minimum from any starting point for deterministic non-convex objectives, where classical optimizers like ___ can diverge when applied with fixed step size. % todo: fill in the blank and make sure this is correct. also split into two sentences so its not a run on. 

We first define the stochastic minimization problem  $\xstar = \argmin_{x \in \mathbb{R}^n} f_k(x)$, where $f_k$ is an unbiased stochastic estimate of an objective function $f$. % todo think we might need a def of a dataset here? not sure. 
...
% todo: add interpolation condition from vaswani2019painless and make sure it is consistent with the notation used to define the stochastic setting above. 
Because neural networks after often overparameterized, this finding motivated the use of line search in modern deep learning, to much empirical success \cite{vaswani2025armijo, galli2023monotone, fox2024nonmonotone, fox2024glocal}. % todo: this is a bit of a run on. 
\todo{Double check to make sure these are all stochastic setting}.

Line search methods are designed to find an optimal $\stepSizeAt{k}$ along $\updateAt{k}$. % todo: think there should be a more specific or formal way saying this
% todo: do i need to introduce backtracking as a special kind of LS here?
The backtracking Armijo line search selects the largest step size $\stepSizeAt{k}$ that satisfies the \textit{stochastic Armijo condition} \cite{vaswani2019painless}:
\begin{equation}
        f_k(\xAt{k} + \stepSizeAt{k} \updateAt{k}) \le f_k(\xAt{k}) + c \stepSizeAt{k} \inner{\nabla f_k(\xAt{k})}{\updateAt{k}},
    \label{eq:armijo_condition}
\end{equation}
where $c \in (0, 1)$ is a hyperparameter and $\inner{\nabla f_k(\xAt{k})}{\updateAt{k}} < 0$, implying that $\updateAt{k}$ must be a descent direction for $f_k$. $c$ was origionally introduced for the full batch setting to ensure, % todo: not sure why c was introduced. Check nocedal and wright numerical optimization. 
\cite{nocedal2006numerical}, however in the stochastic setting it serves a dual purpose of controlling conservativeness of the condition. Line search methods are flexible in that they can easily be layered atop any optimizer that produces an update vector $\updateAt{k}$ satisfying this property \cite{nocedal2006numerical,vaswani2025armijo,shea2024so}.

A backtracking procedure is used to find $\stepSizeAt{k}$ by evaluating an exponentially decaying sequence of candidate step sizes $\stepSizeAt{k}^{(i)} = \stepSizeInit \beta^i$ for $i = 0, \dots, B$ where $\stepSize_0 > 0$ is some initial step size and $\beta \in (0, 1)$ is a decay factor. The algorithm sets $\stepSizeAt{k} = \stepSizeAt{k}^{(i)}$ using the smallest $i$ satisfying Equation \tocite{ref to armijo codintion above}, typically falling back to $\stepSizeAt{k}^{(B)}$ if no candidate is accepted. The linear complexity of $B$ backtracking evaluations has motivated heuristics to optimize the starting point $\stepSizeInit$. Notably, \cite{vaswani2019painless} proposed to initialize each iteration $k$ of the line search from $\stepSizeAt{k} \delta^{n/b}$, where $\delta > 1$, $n$ is the dataset size, and $b$ is the batch size.
...
% todo: Write the below two sections (Trust Region Methods and The Cauchy Point) with about the same specificity as the line search section above. When writing this section, I have provided n_and_w.txt as a reference (which contains text from the trust region chapter of Nocedal and Wright numerical optimization). n_and_w.txt also contains the abstracts for the relevant background (no need to reference anything outside of these unless i am missing something critical, in which case let me know) but remember this is just a basic background of the funementals, the refs are just for mentioning where this has been used.
\subsection{Trust Region Methods}

\subsubsection{The Cauchy Point}


\Chapter{Parallel Argmin Line Search} \label{sec: pals}
...
\Chapter{Gain Ratio for Adaptive Step Size}

\section{A First-Order Stochastic Trust-Region Method}
\todo{This is the new method that is very similar to mine I think is important to compare to, very simple first order TR}

\section{Adaptive Step Size using the Gain Ratio}
...
\todo{Need to implement the standard first order method to compare to this}
...
\begin{enumerate}
  ...
  \item Note the effcitveness of linesearch despite no garantee that the interpolation condition holds, perhaps it does hold over the on policy distribution since the agent only ever regularly enters a relatively small portion of the state space. 
\end{enumerate}
\section{The Importance of Large Batch Size}
\todo{Note that some LS methods for the non-interpolation have been shown to need large batch size in SL to (from painless): "Other authors [12, 17, 22, 42, 62] use a line-search termination criteria that requires function/gradient evaluations averaged over multiple samples. However, in order to achieve convergence, the number of samples required per iteration (the “batch-size”) increases progressively, losing the low per iteration cost of SGD."}
