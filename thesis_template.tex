...
\section{Gain-Ratio Adaptive Step Size} % Local TODO: make terms and style consistent and formatting consistent with the other pseudocode.
\begin{algorithm}
...
\subsection{Gain-Ratio with a Second Order Model}
Can be thought of as the Cauchy point with the trust region scaled by the update norm.
% make sure to explain that the ratio is averaged but not the search, that is done per update

\section{First-Order Trust Region with an Averaged Ratio}
To isolate for the benefit of the gain-ratio mechanism, we also employ averaged statistics. However, an ablation using all trust region methods with individual updates rather than averaging 

\section{The Cauchy Point with an Averaged Ratio}
\todo{!Important Question for Adam!: I am now realizing that I think it makes more sense for this method to update every timestep rather than only update every n steps based on curvature for a single batch, which seems potentially bad. (in the spirit of this being a baseline i think upating every step makes sense? not sure? I actually haven't tested updating ss every step with GRASS so don't know if it would help or hurt or do nothing). One can also use the current curvature to set the ss each step but only expand the TR every n steps based on average stats. The same question also goes for the first order baseline described in the previous section: Should it update every step to be an exact baseline from the lit or average to be directly comparable with GRASS and make sure that GRASS is good because it doesn't normalize the gradient, rather than being good bc of the averaging of stats? Not sure what is best here.}
\section{Globalization Strategies with Momentum-Based Optimizers}
As both line search and trust region rely on descent directions as input, combining them with momentum-based optimizers that may produce ascent directions is non-trivial. Several methods have been proposed for combining line search methods with momentum in supervised learning \tocite{citations}, however there remains no clear consensus regarding the most effective approach. Additonally, to the best of our knowledge, this issue has not been examined in RL nor with trust region methods \todo{confirm this}. Resolving this challenge is especially important in deep RL, where the Adam optimzier, which uses a bias-corrected form of Polyak momentum, is ubiquitous \tocite{citation}. To combine globalization strategies with the Adam in RL, we adopt an approach used for combining line search with Polyak momentum in \cite{vaswani2019painless}, whereby momentum is not used in the search direction but still used in the applied update. We introduce an analogous approach for trust region methods, whereby momentum is not applied when computing the gain-ratio or determining $\tau$ for the Cauchy point \todo{make sure tau is consistent with cauchy psuedocode} however momentum is still used in the applied update. 
% todo: Simple general form pseudocode for this approach with adam and an adaptive step size method. Form should be the adam update vec without momentum is fed to the step size method and then it produces the lr, and then the update with momentum is applied. Note that for cauchy it is identifying lr and tau. I am not sure what the best way to account for this is in a clean way. 
...
For the Armijo linesearch, we apply an EMA on the chosen step size as described for PALS in % local todo: ref to pals section
, with decay rate %todo: latex for the symbol we used for this
$= 0.99$ for both algorithms.
