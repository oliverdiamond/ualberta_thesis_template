...
\usepackage{placeins}  %% <- for \FloatBarrier, used to keep algorithms and tables
                       %%    inside the section that introduces them
\usepackage{nicefrac}
\usepackage{xcolor}
\usepackage{amsmath}
\usepackage{amssymb}
...
I am immensely grateful to my advisor, Professor Adam White, for his steadfast encouragement, insightful counsel, and grounding presence throughout the research process over the past two years. My gratitude also goes to my primary collaborator, Samuel Neumann, for his extensive contributions to this project, and for being an inspiring and generous mentor throughout trails of graduate research. Special thanks are also due to Professor Martha White and Dr. Chen Fan for lending their astute perspectives at many key stages of this work. I also wish to thank my labmates for their friendship and countless thought-provoking conversations. Lastly, thank you to Mom, Dad, and Julian for being there for me every step of the way.
...
In practice, systematically tuning step size is common for deep RL algorithms. Perhaps the most common approach is a grid search, where a wide range of hyperparameter combinations are tested. Grid searches can be effective but typically require parallelism allowed for by a simulator, limiting their applicability. Further these broad hyperparameter searches are not generally possible for agents deployed in real-world settings such as industrial control \cite{luo2022controlling,janjua2024gvfs}, where testing multiple
hyperparameter configurations can be time-consuming or dangerous. Finally, grid searches typically find a fixed step size that works well on average throughout learning and across runs -- not a step size tailored to a specific environment on a specific time step.
...
Globalization strategies have been utilized much less in deep reinforcement learning, though line search methods have been employed in the tabular RL setting \tocite{Grab from long list before this}. Trust region methods have been used for control in deep RL with TRPO \cite{schulman2015trust}, which uses a backtracking line search to keep each update within a KL trust region in policy-distribution space. However, to the best of our knowledge, neither line search nor trust region methods have been used to adapt the step size for an underlying optimizer with respect to the objective in deep RL control.
...
\begin{itemize}
\item \textbf{Chapter~\ref{sec: background}} provides a formal treatment of the RL problem, describes several canonical RL algorithms along with their deep RL counterparts, and defines line search and trust region methods.
\item \textbf{Chapter~\ref{sec: methods}} introduces PALS and GRASS, as well as several independent modifications to stabilize the step sizes selected by globalization strategies in high-variance regimes like RL. It also describes how all the examined methods can be combined with momentum-based optimizers and Sharpness Aware Minimization.
\item \textbf{Chapter~\ref{sec: benchmarking}} investigates the performance of all the examined adaptive methods with DQN and PPO, comparing against tuned fixed step sizes and schedules.
...
This work is only concerned with episodic MDPs. In the episodic variant of the RL problem, the agent-environment interaction loop is divided into a sequence of episodes, each of which ends when the agent reaches a special terminal state $S_T$ as determined by $\dynamics$. The terminal state always transitions to a start state sampled from $\startStateDistribution$. A single episode produces a trajectory of experience $\tau \doteq (S_0, A_0, R_1, S_1, A_1, R_2, \dots, S_{T-1}, A_{T-1}, R_T, S_T)$, where $S_T$ is terminal. The discounted return from timestep $t$ is the discounted sum of the rewards collected from that step until the end of the episode,
\begin{equation}
    G_t \doteq \sum_{k=0}^{T - t - 1} \discount^k R_{t + k + 1}.
    \label{eq:return}
\end{equation}
The agent's objective is to maximize the expected discounted return from the starting state, defined as $J(\pi) \doteq \mathbb{E}_\pi \left[ G_0 \right]$. The task of discovering an optimal policy $\pi^* \doteq \arg\max_\pi J(\pi)$ is referred to as the \textit{control} problem.

\section{Value Functions}
To learn a policy that maximizes return, RL algorithms often rely on value functions. The state-value function $\vfn: \stateSpace \to \RR$ is the expected return when starting in some state $s$ and taking actions according to a policy $\pi$ thereafter, defined as $\vfn(s) \doteq \mathbb{E}_\pi \left[ G_t \mid S_t = s \right] $. Similarly, the action-value function $\qfn: \stateSpace \times \actionSpace \to \RR$ gives the expected return for taking an action $a$ in a state $s$ and then once again following $\pi$, defined as $ \qfn(s, a) \doteq \mathbb{E}_\pi \left[ G_t \mid S_t = s, A_t = a \right] $. 
Both of these functions have recursive definitions given by the Bellman equations for $\vfn$ and $\qfn$ \tocite{Bellman}: 
\begin{align}
    \vfn(s) &\doteq \E_\pi \left[ R_{t+1} + \discount \vfn(S_{t+1}) \mid S_t = s \right] \nonumber \\
    &= \sum_{a} \pi(a \mid s) \sum_{s'} \dynamics(s' \mid s, a) \sum_{r} \rewardFunction(r \mid s, a, s') \left( r + \discount \vfn(s') \right),
    \label{eq:bellman_v} \\
    \qfn(s, a) &\doteq \E_\pi \left[ R_{t+1} + \discount \qfn(S_{t+1}, A_{t+1}) \mid S_t = s, A_t = a \right] \nonumber \\
    &= \sum_{s'} \dynamics(s' \mid s, a) \sum_{r} \rewardFunction(r \mid s, a, s') \left( r + \discount \sum_{a'} \pi(a' \mid s') \qfn(s', a') \right),
    \label{eq:bellman_q}
\end{align}

\subsection{Value Function Approximation} 
Reinforcement learning methods often learn estimates of $\vfn$ and/or $\qfn$, as the true values are typically unknown to the agent. We define these estimates as parameterized functions $\vparam \in \mathbb{R}^k$ and $\qparam \in \mathbb{R}^j$ where $\vparams$ and $\qparams$ are learnable parameters. If $|\stateSpace|$ (or $|\stateSpace \times \actionSpace|$ in the case of action-value estimation) is sufficiently small, $\vparams$ and $\qparams$ can be the entries of tables with one row per state or state-action pair, allowing for exact estimation of $\vfn$ and $\qfn$, respectively. This is known as the \textit{tabular} setting, which underlies many of the foundational theoretical results in RL. 

However, for problems in which $\stateSpace$ is massive, like navigating a robot from pixel-based states, maintaining a table is not possible due to memory constraints. The algorithm can therefore only learn an approximation of $\vfn$ or $\qfn$. In this case, the designer specifies a set of $d$ feature functions $\varphi_1, \dots, \varphi_d$, each mapping states or state-action pairs to a scalar $\varphi_i : \stateSpace \to \RR$, that allow for generalizing value estimates over large subsets of $\stateSpace$. This setting, known as RL with function approximation (FA), has two important variants: linear FA and non-linear FA. In the linear case, the features serve as a typically non-linear, fixed-basis representation for an approximator that is linear in $\vparams$ or $\qparams$. In the non-linear case, the features are often a set of identity functions corresponding to the dimensions of $\stateSpace$, and the approximator is non-linear in $\vparams$ or $\qparams$. The tabular setting is actually a special case of linear FA with $|\stateSpace|$ features corresponding to indicator functions for each state $s \in \stateSpace$. Because of its generality and importance in modern RL, we present all algorithms in this work for the FA setting.
N
...
\begin{equation}
    \text{MSVE}(\vparams) \doteq \sum_{s \in \stateSpace} \stateDistribution(s) \left( \vfn(s) - \vparam(s) \right)^2,
    \label{eq:msve_v}
\end{equation}
or when learning action values as
\begin{equation}
    \text{MSVE}(\qparams) \doteq \sum_{s \in \stateSpace} \stateDistribution(s) \sum_{a \in \actionSpace} \pi(a \mid s) \left( \qfn(s, a) - \qparam(s, a) \right)^2,
    \label{eq:msve_q}
\end{equation}
where $\stateDistribution$ is the on-policy state distribution introduced above.
In the tabular setting, if $\dynamics$ and $\rewardFunction$ are known, this objective can be minimized by iteratively solving the systems of Bellman equations defined by Equations~\eqref{eq:bellman_v} or ~\eqref{eq:bellman_q} via dynamic programming \cite{sutton1998introduction}. When $\dynamics$ and $\rewardFunction$ are unknown, policy evaluation becomes an online stochastic optimization problem \todo{citation}, as algorithms must construct estimates of $\vfn$ and $\qfn$ from stochastic trajectories and then minimize the MSE. Once the update targets are obtained, any optimization algorithm may be employed to reduce the loss, however all pseudocode presented in the the next two sections uses stochastic gradient descent (SGD). 
...
Bootstrapping yields a biased, yet lower variance estimate of the true value function. Moreover, on-policy TD(0) with linear function approximation is still guaranteed to converge to the TD fixed point (the best approximation representable by the features) under standard stochastic approximation conditions on the step size sequence. Using bootstrapped targets in place of the full return has the added benefit of allowing for immediate updates after each transition in the environment rather than waiting till the end of the episode. However, a key downside of TD(0) relative to Monte-Carlo is that it can take many episodes for rewards attained at the end of a trajectory to propagate back into the value estimates for the early states that led to it.
\FloatBarrier
\subsection{$\lambda$-Returns}
The TD(0) update outlined above has a more general form. The agent may actually observe $N$ rewards before bootstrapping, which reduces the bias introduced by using $\vparam$ in place of $\vfn$ and speeds up credit assignment. This target is referred to as an $N$-step return,
\begin{equation}
...
As the policy used for gathering data differs from the greedy policy being evaluated, Q-learning is classified as an \textit{off-policy} learning algorithm. It allows for the sustained exploration required for convergence in the tabular setting, while maintaining the ability to learn $\pi^*$ via greedy policy updates. A key benefit of off-policy learning is the ability to learn from experience generated by any policy, which proves especailly useful for its use with deep neural networks (DNNs) discussed in Section~\ref{subsec:dqn}. 
...
\begin{equation}
    \nabla_\theta J(\theta) \doteq \E_\pi \left[ G_t \frac{\nabla_\theta \pi(A_t \mid S_t, \theta)}{\pi(A_t \mid S_t, \theta)} \right].
    \label{eq:pg}
\end{equation}
Intuitively, moving $\theta$ in this direction increases the probability of actions with high value. One advantage of this approach over value-based methods is that a well-performing policy may be simpler to approximate than its value function. For example, an optimal policy might require only a simple linear decision boundary to separate good actions from bad ones, whereas the value function itself may be highly non-linear. An additional benefit of directly parameterizing $\pi_\theta$ is the ability to handle pseudo-continuous action spaces. While technically finite on computer hardware, many action spaces are so large and dense that they are best treated as continuous. This setting renders value-based methods like Q-learning intractable, as they require computing a maximum over all actions for policy improvement. Policy gradient methods bypass this by learning a probability density $p_\theta(\cdot \mid s)$ over $\mathbb{R}^m$, allowing the agent to efficiently sample $A_t \sim p_\theta(\cdot \mid S_t)$.
...
\begin{equation}
L^{\text{CLIP}}(\theta) \doteq \mathbb{E}_t\!\left[ \min\!\left( r_t(\theta)\,\hat{A}_t,\; \operatorname{clip}\!\left(r_t(\theta), 1-\epsilon, 1+\epsilon\right)\hat{A}_t \right) \right],
\end{equation}
...
\chapter{Globalization Strategies in Deep Reinforcement Learning} \label{sec: methods}
In this chapter we first introduce two first-order methods for adapting the step size parameter in RL: Parallel Argmin Line Search (PALS) and Gain-Ratio Adaptive Step Size (GRASS). These approaches build upon backtracking line search and trust region methods. We also introduce several independent modifications to stabilize the chosen step sizes in high-variance regimes like RL. We show that these modifications can also be applied with backtracking Armijo line search, as well as standard first-order trust region and the Cauchy point method. We then outline an approach for using globalization strategies to select step size for momentum-based optimizers. Lastly, we describe sharpness aware minimization and demonstrate how it can be easily applied in tandem with globalization strategies.

\section{Parallel Argmin Line Search} \label{sec: pals}
We introduce a new line search algorithm that employs a parallel search to avoid both sequential backtracking and common initialization heuristics \cite{vaswani2019painless,galli2023monotone}.\footnote{PALS was developed by Samuel Neumann in co-authored, in-prep work.} This results in a more effective selection criterion based on the step size that provides the greatest improvement in the objective.
...
\begin{equation}\label{eq_armijo_sgd}
    \stepSizeAt{k} \doteq \arg\min_{\stepSize \in \mathcal{H}_k} f_k(\xAt{k} + \stepSize \updateAt{k})
\end{equation}
...
This same first-order trust-region approach (without the median gain-ratio) has been used with standard SGD in supervised learning \tocite{2026 Preprint that does simple first order TR in SL} but not extended to work in tandem with arbitrary optimizers as described in Section~\ref{subsec:trust_region} nor applied in RL. 
...
Trust region methods face a related but distinct difficulty in that they depend on constructing the local model from a descent direction. Let $\hat{u}_k \doteq -A_k^{-1} \nabla f_k(\xAt{k})$ denote the no-momentum direction and $u_k \doteq -A_k^{-1} m_k$ the momentum direction, and define
\begin{equation}
\begin{aligned}
    \bar{u}_k &=
    \begin{cases}
...
\end{aligned}
\end{equation}
The step size $\stepSizeAt{k}$ is adapted via the gain ratio computed from $\bar{u}_k$, following Algorithm~\ref{alg:grass}. The Cauchy point scaling is $\tau_k = 1$ for GRASS and the first-order trust region method, and for the standard Cauchy point method is given by
\begin{equation}
    \tau_k =
    \begin{cases}
...
\end{equation}
The above follows the same derivation as Equation~\eqref{eq:cauchy_tau} using the slope and curvature along the unit direction $\bar{u}_k$ in place of those of the negative gradient direction. The parameter update is then taken along the momentum direction:
\begin{equation}
...
Gradient descent with a sufficiently large step size has been observed to enter a regime known as the edge of stability (EOS), where the sharpness of the loss landscape stabilizes near a threshold determined by the step size rather than continuing to grow \cite{cohen2021gradient}. Sharpness here is defined as the maximum eigenvalue of the Hessian $\lambda_{\max}(\nabla^2 f(x))$, or the maximum eigenvalue of the preconditioned Hessian $\lambda_{\max}(A^{-1}\nabla^2 f(x))$ when using optimizers that employ a diagonal preconditioner $A$ such as RMSProp and Adam \cite{cohen2024adaptive}. The maximum eigenvalue measures the curvature of the objective along the direction with the largest second directional derivative, so a large $\lambda_{\max}$ means the loss rises steeply for a small displacement in at least one direction, as shown in Figure~\ref{fig:precond_sharpness}a. The definition changes when applying the preconditioner $A^{-1}$ to the gradient, as with $A$ held fixed this is equivalent to gradient descent on the preconditioned objective $\tilde{f}(y) = f(A^{-1/2} y)$ in the transformed coordinates $y = A^{1/2} x$. The Hessian of $\tilde{f}$ is $A^{-1/2} \nabla^2 f(x) A^{-1/2}$, which has the same eigenvalues as $A^{-1} \nabla^2 f(x)$ \tocite{citation}, so the preconditioned sharpness corresponds to the sharpness of the objective the optimizer is effectively minimizing. Both the preconditioned sharpness and the high-curvature direction from which it is derived may not correspond to that of the original coordinates, as shown in Figure~\ref{fig:precond_sharpness}. We approximate $\lambda_{\max}$ for the preconditioned Hessian using power iteration on $A^{-1/2} \nabla^2 f(x) A^{-1/2}$. \tocite{citation}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/thesis/methods/precond_sharpness.png}
    \caption[Effect of preconditioning on sharpness.]{Effect of preconditioning on sharpness.
    \textbf{Left:} Contours of a quadratic $f$ with
    $\nabla^2 f = \mathrm{diag}(10,1)$, whose curvature is largest along $x_1$. 
    \textbf{Right:} The same function in the transformed coordinates $y = A^{1/2}x$ with
    $A = \mathrm{diag}(20,1)$, where $\nabla^2 \tilde{f} = \mathrm{diag}(0.5,1)$. The
    sharpest direction is now $y_2$ rather than $y_1$. Since $A^{-1/2}\nabla^2 f(x)A^{-1/2}$
    has the same eigenvalues as $A^{-1}\nabla^2 f(x)$, the preconditioned sharpness
    corresponds to the sharpness of the objective the optimizer is effectively minimizing.}
    \label{fig:precond_sharpness}
\end{figure}

Reaching the EOS is typically desirable, as flatter regions of the loss landscape are hypothesized to generalize better than sharp ones \tocite{citation for flat minima generalization}. In the full-batch supervised setting, line search algorithms have been shown to sometimes select highly conservative step sizes and fail to reach the EOS \cite{roulet2024stepping}, resulting in slow convergence to sharp, poor-performing local minima. However, the effect of loss landscape sharpness on the performance of globalization strategies has not been studied in RL.

Sharpness aware minimization (SAM) is an optimization technique that incentivizes convergence to flatter regions of parameter space. SAM was introduced to directly penalize sharp minima by replacing the ordinary loss with a worst-case loss over a small neighborhood around the current parameters,
\begin{equation}
    f_k^{\text{SAM}}(x) = \max_{\norm{\epsilon} \le \kappa} f_k(x + \epsilon),
    \label{eq:sam_loss}
\end{equation}
where $\kappa > 0$ is the neighborhood radius. Directly solving the inner maximization is typically intractable, so SAM instead approximates it with a single gradient ascent step, yielding a perturbation
\begin{equation}
    \hat{\epsilon}(x) = \kappa \frac{\nabla f_k(x)}{\norm{\nabla f_k(x)}},
    \label{eq:sam_epsilon}
\end{equation}
$\nabla f_k^{\text{SAM}}(x)$ is therefore approximated with $\nabla f_k(x + \hat{\epsilon}(x))$, treating $\hat{\epsilon}(x)$ as a constant with respect to $x$ (i.e., its own gradient is stopped). This approximation is reasonable because the dropped term involves higher order derivatives of the loss, so it likely has little influence on the update direction for the small radii used in practice \tocite{SAM paper}.
Pseudocode for a single SAM update is given in Algorithm~\ref{alg:sam}.
...
SAM has been used to achieve better generalization in offline RL \tocite{citation} and for mitigating plasticity loss in online RL \tocite{citation}, however, to the best of our knowledge, it has not been studied as a means of addressing failure modes of globalization methods in RL nor supervised learning.


\chapter{Comparing Globalization Strategies to Fixed Step Sizes and Schedules} \label{sec: benchmarking}
In this chapter, we examine the performance of the adaptive step size methods outlined above with DQN and PPO in RL environments with continuous and discrete action spaces of varying complexity. We found that all adaptive methods performed well in the classic control suite, demonstrating that globalization strategies can serve as an effective means of automatic step size selection in deep RL. In more complex environments, we found that GRASS significantly outperformed the other adaptive methods with DQN in Atari, with PALS ranking second by a substantial margin. However, both methods still fall short of the tuned baselines at either batch size, as globalization strategies appear to exhibit several distinct failure modes in this setting. Additionally, only GRASS and PALS consistently matched the performance of tuned baselines with PPO in Brax, demonstrating their robustness compared to standard line search and trust region methods. 

\section{Adaptive Step Size with DQN} \label{sec:dqn_results}
We first examine the use of globalization strategies for adaptive step size with DQN in the classic control suite \tocite{citation}. These environments consist of low-dimensional state spaces describing the position and velocity of a small mechanical system, with actions controlling an applied force or torque that drives the system's dynamics. While the classic control environments are traditionally defined with continuous actions, their action space may also be discretized for use with discrete-action algorithms like Q-learning. Specifically, we examine variants of the Acrobot, CartPole and MountainCar environments implemented as modifications of the corresponding \texttt{gymnax} \tocite{gymnax} environments, with modifications made to bring their dynamics closer to those described in \cite{sutton1998introduction}. These modifications are detailed in Appendix~\ref{app_classic_control_diffs}.

We compare the performance of GRASS, the first-order trust region method outlined in Section~\ref{sec:grass} (henceforth referred to as trust region), the Cauchy point method, PALS, backtracking Armijo line search, and tuned fixed step sizes. The same set of methods is examined in every experiment in this work, and all settings described below are held constant across every algorithm and environment pairing unless otherwise noted.

For trust-region-based methods, we use standard values from the classical optimization literature \cite{nocedal2006numerical} \tocite{other source that establishes 0.5 as common} of $(T_{\text{lo}}, T_{\text{hi}}) = (0.25, 0.75)$ and $(\alpha_{\text{shrink}}, \alpha_{\text{grow}}) = (0.5, 2)$. Though setting $\alpha_{\text{shrink}}$ to $0.25$ is also common, we use $0.5$ as this produces smaller, more conservative changes in $\stepSize$, which may be more appropriate for the high-variance regime of online RL. We set $\trustRegionInit$, the initial step size for all trust-region-based methods, to $1\mathrm{e}{-5}$ in all experiments. For $K$, we use $\nicefrac{1}{4}$ of the total updates performed between target network refreshes. All methods utilize the median gain-ratio introduced in GRASS, though we present an ablation comparing this approach against the traditional single-step ratio in Section~\ref{sec:gain_ratio_ablation}. None of the hyperparameters listed above were tuned. As the gain-ratio is invariant to the scale of the loss and gradients, these default values have been used successfully across convex, non-convex, constrained, and stochastic trust region methods \cite{nocedal2006numerical} \tocite{other sources}. 

For all line search methods, we start the search at $1$ to capture a wide range of values and use a sufficient decrease coefficient of $c=0.1$, which is common for stochastic line search in supervised learning \cite{vaswani2019painless}. We use $B=30$ backtracking steps with a decay factor of $\beta=0.7$, which places the smallest candidate step size at roughly $2\mathrm{e}{-5}$. This value is well below the tuned fixed step sizes described below, preventing implicit tuning of the step size for these methods. For both PALS and the Armijo line search, we apply an EMA on the chosen step size as described in Section~\ref{sec: pals}, with decay rate $\zeta = 0.99$. As with the trust region methods, we do not tune the line search hyperparameters to avoid replacing one tuned parameter with another. All hyperparameters for the line search and trust region methods are given in Appendix~\ref{app_globalization_hypers}.

We use the Adam optimizer in all experiments. We set Adam's hyperparameters according to the Optax library defaults \tocite{optax} and report them in Appendix~\ref{app_hyperparams}. All adaptive step size methods employ the approach for handling momentum described in Section~\ref{sec:momentum}. When using Adam with a fixed step size, we select $\stepSize$ using cross-environment-tuning \tocite{paper for this} by conducting grid search over a range of candidate step sizes and selecting the one with the best mean normalized performance across all environments in the suite. We perform tuning separately for each examined batch size, and we refer to a tuned fixed step size with a given batch size $b$ as \textit{Tuned-$b$}. Ranges used in the grid search and the selected fixed step size for all algorithms and environment suites are given in Appendix~\ref{app_tuning_ranges}.

For DQN, we use the methods outlined above to update the parameters of an action-value network $\qparam$. We use an MLP with two hidden layers of 128 and 80 units, respectively, and ReLU activations. For all adaptive methods, we use a larger batch size of 1024 to reduce the variance of the gradient estimates used in determining step size. To facilitate fair comparison, for fixed step sizes we use both this batch size and a more standard DQN batch size of $32$. All other algorithm hyperparameters match the defaults from \tocite{PureJaxRL repo} and are reported in Appendix~\ref{app_hyperparams}. 

To assess algorithm performance, we run the standard RL interaction loop described in Chapter~\ref{sec: background} for 150 thousand timesteps. We average results over 20 seeds, which control the initialization of the agent's learnable parameters and the starting state of the environment. To assess performance for a single run, we use the mean episodic return \tocite{epirical design in RL}, defined as the average return across all episodes, weighted by episode length:
\begin{equation}
    \text{Mean Episodic Return} = \frac{\sum_{i=1}^{M} T_i G^{(i)}}{\sum_{i=1}^{M} T_i},
    \label{eq:weighted_performance}
\end{equation}
where $M$ is the total number of episodes completed in the run, and $T_i$ and $G^{(i)}$ are the length and return of episode $i$, respectively. We may either examine this value directly or assess performance qualitatively by analyzing the episodic return over time, which we refer to as a \textit{learning curve}. 

Figure~\ref{fig:dqn_classic_adam} shows the learning curves and step size $\stepSize$ over time for the classic control suite. Note that we use $\stepSize$ and $\Delta_k$ of Section~\ref{subsec:trust_region} interchangeably, as $\Delta_k$ serves the role of the step size in the update for GRASS and in the normalized update for the first-order trust region method. Similarly, for the Cauchy point method we report step size as $\tau_k \Delta_k$. We found that all the examined methods performed roughly equally in all three environments. At the beginning of learning, step sizes for GRASS and the trust region methods quickly rose from their starting point $\trustRegionInit$ to a peak and then decayed thereafter. Step sizes for standard Trust Region and the Cauchy point were larger because the effective step size for these methods is actually $\Delta_k / \norm{\updateAt{k}}$ and $\tau_k \Delta_k / \norm{\updateAt{k}}$, respectively. The line search methods exhibited a similar pattern of decay throughout the run. While the classic control environments are not particularly sensitive to step size \todo{cite}, these results highlight globalization strategies as a promising mechanism for automatic step size selection in deep RL.
...
Turning to more complex MDPs, we now examine DQN in the Arcade Learning Environment \tocite{ALE}. In the ALE, the agent receives pixel-based frames of Atari 2600 games, yielding a higher-dimensional state space than classic control. This makes learning an accurate value function or policy a substantially harder optimization problem. The ALE has many configurable parameters controlling the environment dynamics. We borrow all environment settings from the Google Dopamine baselines \tocite{Dopamine, Revisiting rainbow}, including sticky actions for stochastic transitions, a minimal action space per game, a frameskip of 4, and a framestack of the previous 4 $84{\times}84$ downsampled and greyscaled frames to compose the state. The full set of used ALE parameters are outlined in Appendix~\ref{app_ale_params}. In this work we only examine the Atari-5 \tocite{atari 5}, a subset of the ALE that is reflective of performance on the full 57 game suite. 

Several implementation details differ from the classic control experiments above. For $\qparam$, we use the standard DQN convolutional network for the ALE introduced in \tocite{nature DQN}. To reduce computational cost, the adaptive methods use a batch size of 512 rather than 1024. For the same reason, the line search methods use $B=10$ backtracking steps with a decay factor of $\beta=0.2$, placing the smallest candidate step size at roughly $1\mathrm{e}{-7}$, which is again well below the tuned fixed step sizes given in Appendix~\ref{app_tuning_ranges}. All other algorithm and Adam hyperparameters match the Dopamine baselines \tocite{Dopamine}, which we report in Appendix~\ref{app_hyperparams}. We run each method for 50 million timesteps \tocite{marlos paper} and average results over 3 seeds.

To calculate a method's aggregate performance across the suite, we first normalize the mean episodic returns of all runs within each environment using Winsorized Min-Max Normalization ($5^{\text{th}}$ and $95^{\text{th}}$ percentiles). For each method, we average these normalized scores across its runs per environment, and then average across all environments to yield the overall mean normalized performance. We use the $5^{\text{th}}$ and $95^{\text{th}}$ percentiles to prevent outliers from distorting the min-max normalization; this does not alter the set of runs used in our analysis. We report 95\% confidence intervals for this metric using a stratified bootstrap with $5{,}000$ resamples.

We found the results in this setting to be much more nuanced than in classic control. Figure~\ref{fig:dqn_atari_norm_perf} shows mean normalized performance across the Atari-5. GRASS greatly outperformed all other adaptive step size methods, with PALS ranking second by a clear margin. However, all adaptive methods performed worse than Tuned-32 and Tuned-512. Nevertheless, GRASS attained this comparatively strong performance using substantially less compute than the line search methods, as the median gain-ratio requires only two loss evaluations per update rather than the $B+1$ evaluations used by PALS.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.45\textwidth]{figures/thesis/performance/2_dqn_atari/dqn_atari_norm_perf.png}
    \caption[Normalized performance of DQN with Adam on the Atari-5.]{Normalized performance of DQN with Adam on the Atari-5. GRASS greatly outperformed the other adaptive methods, with PALS ranking second by a clear margin. Neither method matched Tuned-512 or Tuned-32.}
    \label{fig:dqn_atari_norm_perf}
\end{figure}

Examining the learning curves in Figure~\ref{fig:dqn_atari_adam}, we found that performance in the Atari-5 was much less uniform across methods and environments than in the classic control domains. For example, in Battle Zone, all adaptive methods except GRASS exhibited either a total failure or a clear collapse in return after some period of learning progress. Figure~\ref{fig:dqn_atari_battle_zone} shows these collapses in performance were correlated with steadily increasing step size, displacement in weight space produced by the update, loss, and Q-values. Starting from the same $\trustRegionInit$ of $1e-5$, we observe that displacement was larger for GRASS and the step size reached a threshold where the gain-ratio failed to match $T_{\text{lo}}$ very early in learning. $\stepSize$ for GRASS promptly shrank and stabilized within an order of magnitude of Tuned-512, while $\stepSize$ continued to grow for the other methods, even as the displacement far exceeded the largest value observed for grass. Step sizes for the line search methods were more stable initially, but then later in the run exhibited the same increasing behavior as the trust region methods. 
...
\begin{figure}[htbp]
    ...
    \caption[Metrics for DQN in Battle Zone.]{Return, displacement, step size, loss, and Q-values for DQN with Adam in Battle Zone. The collapse in return for all adaptive methods except GRASS was correlated with steadily increasing step size, displacement, loss, and Q-values.}
    \label{fig:dqn_atari_battle_zone}
\end{figure}

It is unclear why GRASS did not exhibit the same instabilities observed for the other adaptive methods. Note that for the standard trust region and Cauchy methods, $\stepSize$ scales a unit vector rather than the raw update, so the displacement per update is $\stepSize$ rather than $\stepSize \norm{\updateAt{k}}$. This explains why the displacement was larger for GRASS initially any why the other methods grew $\stepSize$ to a larger value, however the displacement for these methods quickly exceeded the largest value observed for GRASS without triggering a decrease in $\stepSize$. This suggests whatever mechanism triggered the massive growth in $\stepSize$ may have originated in this early period in learning when the trust region and Cauchy point methods require more updates to reach the same displacement as GRASS. Additionally, unlike GRASS, the examined line search methods adjust the step size each update and always consider the maximum step of $1$, potentially making them more prone to all the instabilities discussed in this section. 
...
We now examine adaptive step size for PPO, beginning with the continuous action versions of the classic control suite. We select step size separately for the actor and the critic. For both the actor and critic networks, we use the standard PPO MLP: two hidden layers of 64 units with tanh activations \cite{shengyi2022the37implementation}. The actor network outputs the mean and standard deviation for a Gaussian distribution with diagonal covariance from which actions are sampled. As with DQN, the adaptive methods use a batch size of 1024, while the fixed step size baselines use either this batch size or the standard batch size of 64. When using a fixed step size with PPO, we also linearly decay the step size over the agent's lifetime following the standard protocol outlined in \cite{shengyi2022the37implementation}. Following \cite{shengyi2022the37implementation}, we use $10$ epochs per update phase with $2048$ transitions per data collection phase. All other PPO hyperparameter settings are set according to \cite{shengyi2022the37implementation} and are given in Appendix~\ref{app_hyperparams}. All line search and trust region hyperparameters are the same as in the classic control experiments from Section~\ref{sec:dqn_results}, with the exception of the window size $K$ for the median gain-ratio, which we set to $\nicefrac{1}{4}$ of the total updates performed after each data collection phase. We again run for 150 thousand timesteps and average results over 20 seeds.

Similarly to DQN, Figure~\ref{fig:ppo_classic_adam} shows that all methods performed roughly equally well, matching the tuned baselines. Chosen step sizes for line search methods and GRASS lay roughly within an order of magnitude above the tuned step size, while those of standard trust region methods were higher, as they normalize the update vector.

We now move to the Brax suite, which provides a collection of continuous-control locomotion tasks originally simulated with the MuJoCo physics engine \tocite{brax}. Like the ALE, these environments all have high-dimensional state spaces, making learning policies and value functions a more challenging optimization problem than in the classic control environments. Unlike the ALE, the Brax environments use proprioceptive states rather than pixel-based and also posses continuous action spaces. State dimensionality ranges from under 10 in the simplest environment to several hundred in the most complex, while action dimensionality ranges from 2 to 17. We examine a subset of five environments from the full Brax suite, including: Ant, HalfCheetah, Swimmer, Humanoid, and Walker2d. Adam hyperparameters here follow the OpenAI baselines settings \tocite{37 details, open ai baselines}, and we run each method for 3 million timesteps \tocite{open ai baselines}, again averaging results over 20 seeds. All other settings match those used with PPO in classic control, as outlined in Appendices \ref{app_hyperparams} and \ref{app_globalization_hypers}. 
...
As with DQN, Figure~\ref{fig:ppo_mujoco_adam} shows that GRASS and line search methods fell within an order of magnitude of the baseline while the Trust Region methods had larger step sizes. Notably, no methods exhibited either the divergent or the overly conservative step sizes observed in some Atari environments with DQN. 

We hypothesize this is primarily a result of using PPO rather than a product of the Brax environments, for several reasons. First, unlike the single-step Q-learning loss used by DQN, the critic targets in PPO are computed using $\lambda$-returns, which place most of their weight on observed rewards rather than the value estimates produced by the preceding update phase. This may alleviate the hypothesized instabilities related to single-step targets discussed in Section~\ref{sec:dqn_results}. The critic target also involves no maximization over actions, so it does not contain the inherent upward bias that can result in overestimation with DQN \todo{cite double q paper}. Additonally, the buffer of $2048$ transitions collected by PPO is likely more on-policy than the large replay buffer of $1{,}000{,}000$ transitions maintained by DQN, suggesting that the interpolation condition of Equation~\eqref{eq:interpolation} may be more likely to hold. The RL objective is defined over the on-policy state distribution, so a batch drawn from a buffer containing data from many past policies is not an unbiased sample of the objective. A stationary point of the true loss therefore need not be a stationary point of each batch loss, violating interpolation. Furthermore, once the likelihood ratio in the policy loss leaves the clipping range, the surrogate objective becomes flat along that direction. As a result, larger steps yields no further improvement, directly capping the step size these methods will accept for the actor. Lastly, the comparatively lower-dimensional proprioceptive states in Brax also likely pose a better conditioned optimization problem than learning from raw pixels, though our experiments do not fully isolate this factor from the choice of algorithm.
...
\begin{figure}[htbp]
    ...
    \caption[Loss landscape approximation for PALS in Name This Game.]{Loss landscape approximation along the update direction for PALS in Name This Game with small and large batch size, for an update performed $10\%$ of the way through learning. The loss was smoother on the small batch, allowing PALS to accept a larger step size.}
...
\end{figure}
In Figure~\ref{fig:adamdir_ppo_mujoco}(b), we found that the actor step sizes for the line search were much larger than the tuned baseline when using $u_k$, while they were within an order of magnitude with $\hat{u}_k$. For GRASS and the trust region methods, the actor step sizes were consistent between the two variants, but the critic step size was larger when using momentum in selection. Figure~\ref{fig:adamdir_ppo_humanoid_loss} shows approximations of the critic loss for PALS in Humanoid. Similarly to DQN, we observed that the larger critic step sizes were a product of smoother loss along the momentum direction, though the actor was less affected.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.72\textwidth]{figures/thesis/ablations/3_adam_direction/adamdir_ppo_humanoid_stepsize_loss.png}
    \caption[Critic loss landscape along momentum and no-momentum directions in Humanoid.]{Approximations of critic loss along the update direction for PALS in Humanoid, with and without momentum in step size selection, for an update performed $10\%$ of the way through learning. The larger critic step sizes with momentum were a product of smoother loss along the momentum direction.}
...
that globalization strategies can select overly conservative step sizes in some environments with DQN, moving into sharp regions of parameter space. We introduce a strategy for resolving this issue using Sharpness Aware Minimization as described in Section~\ref{sec:sam}. When combining SAM with globalization strategies, we set $\kappa$ to the recommended default value of $0.05$ \tocite{SAM paper}, which we found to achieve the best performance via cross-environment-tuning over the Atari-5. The swept values are given in Appendix~\ref{app_tuning_ranges}. Following previous applications of SAM in RL \tocite{atari sam paper}, we also utilize layer normalization \tocite{layer norm} in tandem with SAM. All other settings match those used for DQN in the Atari-5 in Section~\ref{sec:dqn_results}.
\begin{figure}[htbp]
    \centering
...
\begin{figure}[htbp]
    ...
    \caption[Loss landscape for SAM in Qbert.]{Loss landscape approximation along the update direction for a single PALS update in Qbert, with and without SAM, for an update performed $10\%$ of the way through learning. The loss was smoother when using SAM, allowing PALS to accept a step size with a larger resulting displacement.}
...
Indeed, examining Figure~\ref{fig:sam_atari5}(a), we found that while performance across the Atari-5 was improved for PALS, performance for GRASS and Tuned-512 was unchanged due to worsened behavior in some environments, as shown in Battle Zone in Figure~\ref{fig:sam_atari5}(b). Performance for Tuned-32 was also significantly worsened when using SAM, though this method did not exhibit the same relatively high sharpness in the loss as the other methods. 
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/thesis/ablations/1_sam/sam_atari5_return_step.png}
    \caption[SAM vs.\ no SAM on Atari-5: return and step size.]{\textbf{(a) Left:} Normalized performance and step size, and \textbf{(b) Right:} return and step size in Battle Zone, for DQN with Adam on the Atari-5, with and without SAM. Performance was improved for PALS, while GRASS and Tuned-512 were unchanged partially due to worsened behavior in Battle Zone. Tuned-32 performed significantly worse with SAM.}
    \label{fig:sam_atari5}
\end{figure}
In this environment, we observed the steadily increasing step size and corresponding performance degradation seen with other globalization strategies in Section~\ref{sec:dqn_results}. This finding suggests that when the observed issues related to sharp loss are not already present, selecting step size based on the SAM objective can lead to aggressively large, high variance steps that destabilize progress on the true objective.
...
Tables~\ref{tab:dqn_hypers} and~\ref{tab:ppo_hypers} report the algorithm, network, and Adam optimizer hyperparameters for DQN and PPO, respectively. These settings are shared by all methods. For each algorithm we report the batch size used by the adaptive step size methods and those used by the tuned fixed step size baselines described in Section~\ref{sec:dqn_results}.
...
\FloatBarrier
\section{Line Search and Trust Region Hyperparameters} \label{app_globalization_hypers}
Tables~\ref{tab:ls_hypers} and~\ref{tab:tr_hypers} report hyperparameters for the line search and trust-region methods, respectively. None of these values were tuned.
...
\begin{tabular}{@{}l l l@{}}
\toprule
Hyperparameter & DQN-Atari & All Other Settings \\
\midrule
Initial step size $\stepSizeInit$ & 1 & 1 \\
Sufficient decrease coefficient $c$ & 0.1 & 0.1 \\
Backtracking steps $B$ & 10 & 30 \\
Decay factor $\beta$ & 0.2 & 0.7 \\
Smallest candidate step size $\stepSizeInit \beta^B$ & $\approx 1\mathrm{e}{-7}$ & $\approx 2\mathrm{e}{-5}$ \\
EMA decay rate $\zeta$ & 0.99 & 0.99 \\
\bottomrule
\end{tabular}
\caption[Line search hyperparameters.]{Hyperparameters for PALS and the backtracking Armijo line search. All settings are shared across algorithm and environment pairings other than DQN in the Atari-5.}
\label{tab:ls_hypers}
\end{table}
...
\toprule
Hyperparameter & \makecell[l]{DQN \\ Classic Control} & \makecell[l]{DQN \\ Atari-5} & \makecell[l]{PPO \\ Classic Control} & \makecell[l]{PPO \\ Brax} \\
\midrule
Initial step size $\trustRegionInit$ & $1\mathrm{e}{-5}$ & $1\mathrm{e}{-5}$ & $1\mathrm{e}{-5}$ & $1\mathrm{e}{-5}$ \\
Shrink threshold $T_{\text{lo}}$ & 0.25 & 0.25 & 0.25 & 0.25 \\
Growth threshold $T_{\text{hi}}$ & 0.75 & 0.75 & 0.75 & 0.75 \\
Shrink factor $\alpha_{\text{shrink}}$ & 0.5 & 0.5 & 0.5 & 0.5 \\
Growth factor $\alpha_{\text{grow}}$ & 2 & 2 & 2 & 2 \\
Gain-ratio window $K$ (updates) & 37 & 500 & 5 & 5 \\
\bottomrule
\end{tabular}
\caption[Trust region hyperparameters.]{Hyperparameters for GRASS, the first-order trust region method, and the Cauchy point method. All values other than the gain-ratio window $K$ are shared across every algorithm and environment pairing. For DQN, $K$ is $\nicefrac{1}{4}$ of the number of updates performed between target network refreshes, while for PPO it is $\nicefrac{1}{4}$ of the updates performed after each data collection phase.}
\label{tab:tr_hypers}
\end{table}

\FloatBarrier
\section{Grid Search Ranges For Cross Environment Tuning} \label{app_tuning_ranges}
For each environment suite, algorithm, and optimizer, we perform a grid search over the range of candidate step sizes given in Table~\ref{tab:tuning_ranges} to obtain the \textit{Tuned N} baselines described in Section~\ref{sec:dqn_results}, where $N$ denotes the batch size used. Table~\ref{tab:tuned_n} reports the selected fixed step size for each environment suite, algorithm, and optimizer. 
...
\toprule
Env. Suite & Algorithm & Optimizer & SAM & Batch Sizes & Step Size \\
\midrule
Classic Control & DQN & Adam & false & 32, 1024 & $1\mathrm{e}{-3}$ \\
Atari-5 & DQN & Adam & false & 32 & $6.25\mathrm{e}{-5}$ \\
Atari-5 & DQN & Adam & true & 32 & $6.25\mathrm{e}{-4}$ \\
Atari-5 & DQN & Adam & true, false & 512 & $6.25\mathrm{e}{-6}$ \\
Classic Control & PPO & Adam & false & 64, 1024 & $3\mathrm{e}{-3}$ \\
Brax & PPO & Adam & false & 64, 1024 & $3\mathrm{e}{-4}$ \\
\bottomrule
\end{tabular}
\caption[Selected fixed step size for each environment suite, algorithm, and optimizer.]{Selected fixed step size for each environment suite, algorithm, and optimizer with and without SAM. The step size was identical across the batch sizes examined in every setting. Atari-5 DQN was run only with Adam.}
\label{tab:tuned_n}
\end{table}

\FloatBarrier
We tune the SAM perturbation radius $\kappa$ using the range given in Table~\ref{tab:sam_tuning_range}. The search is repeated for PALS, GRASS, and both tuned fixed step size baselines, though the selected value of $0.05$, which matches the default recommended by \tocite{SAM paper}, was the same for all methods.
...
Env. Suite & Algorithm & Grid Search Range \\
\midrule
Atari-5 & DQN & $0.01$, $\mathbf{0.05}$, $0.1$ \\
\bottomrule
\end{tabular}
\caption[Grid search range for the SAM perturbation radius $\kappa$.]{Grid search range for the SAM perturbation radius $\kappa$. The same range is swept for PALS, GRASS, Tuned-32, and Tuned-512.}
\label{tab:sam_tuning_range}
\end{table}
