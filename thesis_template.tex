...
Pseudocode for the backtracking Armijo line search defined in Section~\ref{sec:adaptive} with EMA on the chosen step size is given below:
...
GRASS can also utilize second order information by identifying the Cauchy point along the raw update vector and then scaling its length by $\eta$, rather than first normalizing the update vector to unit length and then scaling by $\eta$. This approach, which we refer to as GRASS Cauchy
...
\begin{equation}
    ...
    \begin{cases}
        ...
        \hat{u}_k / \norm{\hat{u}_k} & \text{for the first-order trust region method and the Cauchy point method,}
    \end{cases}
    \qquad
    d_k =
    \begin{cases}
        u_k & \text{for GRASS and GRASS Cauchy,} \\[4pt]
        u_k / \norm{u_k} & \text{for the first-order trust region method and the Cauchy point method.}
    \end{cases}
\end{equation}
...
The classic control environments are defined for continuous action spaces, however their action space may be discretized by partitioning each action dimension into $n$ evenly spaced values spanning its range and taking the Cartesian product across all dimensions, yielding $n^d$ discrete actions for a $d$-dimensional continuous action space. Each discrete action is an index into this grid of points, which is mapped back to the corresponding continuous action vector before being passed to the underlying environment.
Specifically, we examine variants of the Acrobot, Cartpole and Mountaincar environments based on those outlined \tocite{sutton1998introduction}, with key implementation differences outlined in % local todo: make appendix section for this (dont need to write anything yet and ref it here)
...
The ALE has many configurable parameters controlling the environment dynamics. We borrow all environment settings from the Google Dopamine baselines \tocite{Dopamine, Revisiting rainbow}, including sticky actions for stochastic transitions, a minimal action space per game, a frameskip of 4 frames per timestep, and a framestack of the previous 4 greyscaled frames to compose the state. The full set of ALE environment implementation details are outlined in % local todo: make appendix section with sample of ale parameters, you can derive them by looking at this config: /Users/oliverdiamond/Desktop/alberta/research/linesearch_brax/experiments/exp114_thesis/dqn/atari/trust_region.json and this runner file: /Users/oliverdiamond/Desktop/alberta/research/linesearch_brax/src/train/dqn/dqn_control_atari_ckp.py and this wrapper file: /Users/oliverdiamond/Desktop/alberta/research/linesearch_brax/src/util/ale_wrapper.py
...
GRASS and all the trust-region-based methods examined in this work share the same hyperparameters: the thresholds $\tau_{\text{lo}}, \tau_{\text{hi}}$ and factors $\alpha_{\text{shrink}}, \alpha_{\text{grow}}$ from Algorithm~\ref{alg:grass}, and the update interval $K$.
Across all experiments, for $(\tau_{\text{lo}}, \tau_{\text{hi}})$ and $(\alpha_{\text{shrink}}, \alpha_{\text{grow}})$ we use standard values from the classical optimization literature \cite{nocedal2006numerical}: $(0.25, 0.75)$ and $(0.5, 2)$, respectively.
For $K$, we use $\nicefrac{1}{4}$ of the target network refresh interval $C$ for DQN and $\nicefrac{1}{4}$ of the total updates performed after each data collection phase for PPO. All the examined trust-region-based methods utilize the median gain-ratio introduced in GRASS. However, we present an ablation using the traditional single-step gain-ratio with all methods in % local todo: ref a new appdix section for this
...
None of the hyperparameter values given above were tuned for the examined environments, and they may be considered reasonable default values. 
...
All experiments described in the remaining chapters of this thesis
employ all the examined step size selection methods in tandem with the Adam optimizer, however experiments with RMSProp and SGD are included in Appendices~\ref{app_dqn_control_classic}, \ref{app_dqn_control}, \ref{app_ppo_control_classic}, and \ref{app_ppo_control}.
