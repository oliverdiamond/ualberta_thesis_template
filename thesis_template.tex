...
\section{First-Order Trust Region \todo{Does this one belong in background?}}
...
We use all envrionment settings from \tocite{Revisiting rainbow}, all of which are outlined in % local todo: make appendix section with sample of ale parameters, you can derive them by looking at this config: /Users/oliverdiamond/Desktop/alberta/research/linesearch_brax/experiments/exp114_thesis/dqn/atari/trust_region.json and this runner file: /Users/oliverdiamond/Desktop/alberta/research/linesearch_brax/src/train/dqn/dqn_control_atari_ckp.py and this wrapper file: /Users/oliverdiamond/Desktop/alberta/research/linesearch_brax/src/util/ale_wrapper.py
In this work we only use atari 5 \tocite{atari 5}, a subset of the ALE that is reflective of performance on the full 57 game suite. All settings follow those defined in 
...
We benchmark GRASS (with and without a second order model, which when used we denote as GRASS Cauchy) and PALS against the first-order trust region method outlined in % local todo: ref to section
(henceforth refered to as simply Trust Region), the Cauchy point method (denoted as TR Cauchy), the standard backtracking Armijo linesearch, as well as tuned fixed step sizes. 
...
To ensure consistency, we maintained identical hyperparameters across all experiments unless otherwise noted.
We use line search for both the parameterized policy and state-value network.
...
To select the fixed step size, we perform cross-environment-tuning \tocite{paper for this}. We repeat this process to determine the optimal fixed step-size for both batch sizes 64 and 1024 (denoted PPO-64 and PPO-1024) for each optimizer. For each optimizer, the best examined step-sizes are consistent across both batch sizes: $3^{-2}$ for SGD and $3^{-4}$ for RMSprop. 
