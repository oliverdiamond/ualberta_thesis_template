...
% todo: make sure all the mentions of figures in all the following chapters get converted to actual refs to the figure that it is discussing. Some of the images may already have figures and some may not, but some names have been changed so make sure that we remove any figures for images that no longer exist in the figures dir. Make sure that all figures actually render in the section in which they are mentioned. If you can do this without scanning the rendered pdf please do that to save tokens, and if you need me to check the actual pdf let me know and ill report back what needs fixing. 
\chapter{Benchmarking Globalization Strategies Against Static Step Sizes}
In this chapter, we examine the performance of all the examined adaptive methods with DQN and PPO. For DQN, we identify all adaptive methods perform well in the classic control suite, demonstrating that globalization strategies can serve as an effective means of automatic step size selection in deep RL. Moving to more complex environments, we find that GRASS significantly outperforms the other adaptive methods with DQN in Atari, with PALS ranking second by a substantial margin. However, GRASS still struggles relative to the tuned baseline with smaller batch size, as globalization strategies appear to exhibit several distinct failure modes in this setting. With PPO, we find that all methods perform similarly well in classic control, while only GRASS and PALS consistently match the performance of tuned baselines in Brax.
...
Unlike in Battle Zone, examining Figure % todo: ref to figures/thesis/performance/2_dqn_atari/dqn_atari_qbert_step_sharpness.png
we observe that for Qbert, step sizes are roughly $10^2$ to $10^4$ times smaller than the tuned baselines, and roughly 10 to 100 times smaller than the chosen step sizes in the other Atari environments (see Appendix %todo: ref to plot for figures/thesis/performance/2_dqn_atari/dqn_atari_return_step_sharpness.png in the appendix
...
We now examine adaptive step size for PPO, beginning with the classic control suite. Similarly to DQN, Figure % todo: ref to figures/thesis/performance/3_ppo_classic/ppo_classic_return_step.png
shows that all methods perform roughly equally well, matching the tuned baselines. Chosen step sizes for line search methods and GRASS lie roughly within an order of magnitude above the tuned step size, while those of standard trust region methods are higher as they normalize the update vector.

Moving to Brax, Figure % todo: ref to figures/thesis/performance/4_ppo_mujoco/ppo_mujoco_norm_perf.png
shows that GRASS and PALS perform on par with the tuned baselines across the suite.
Backtracking Armijo nearly matches the baselines, while the standard trust region methods perform worse. Examining the learning curves in Figure % todo: ref to figures/thesis/performance/4_ppo_mujoco/ppo_mujoco_return_step.png
...
% DQN
To examine the effect of batch size on the performance of globalization strategies in RL, Figure % todo ref to fig figures/thesis/ablations/2_batch_size/batch_dqn_atari5_step.png left side, we will denote with (a) in the caption. so say Figure Na. 
compares the normalized performance of GRASS and PALS averaged across the Atari-5 with standard DQN batch size of 32 and the larger batch size of 512 utilized and the preceding experiments. Similar to what has been observed in SL, a larger batch size massively improves the control performance of both methods \cite{roulet2023interplay}, with both GRASS and PALS completely failing to learn with small batch size. Furthermore, figure % todo ref to fig figures/thesis/ablations/2_batch_size/batch_dqn_atari5_step.png right side, we will denote with (b) in the caption. so say Figure Nb. 
shows that with small batch size in Name This Game, the accepted step sizes are roughly 1000 times larger than the tuned baseline, and performance flatlines. At the larger batch size, step sizes are much closer to the fixed baseline, though GRASS appears to select values roughly 100 times smaller than the other adaptive methods. Examining the return in Figure % todo ref to fig figures/thesis/ablations/2_batch_size/batch_dqn_atari5_step.png right side, we will denote with (b) in the caption. so say Figure Nb. 
...
Figure % todo: ref to figures/thesis/ablations/2_batch_size/batch_dqn_name_this_game_stepsize_loss.png
...
We find that the loss on the small batch is smoother than that of the large batch, allowing for a larger displacement. Increasing the batch size reduces the variance of $f_k$, and therefore a decrease measured on an individual batch is a more reliable indicator of progress on the full objective. We observe the accepted step size is tempered accordingly. 
...
% PPO
Turning to PPO in Brax, we again find that all adaptive methods perform much better with large batch size, as shown in Figure % todo ref to fig figures/thesis/ablations/2_batch_size/batch_ppo_mujoco_step.png left side, we will denote with (a) in the caption. 
Figure % todo ref to fig figures/thesis/ablations/2_batch_size/batch_ppo_mujoco_step.png right side, we will denote with (b) in the caption.
...
\end{figure}
We observe that both methods fail entirely with momentum in step selection. Examining Figure % todo ref fig figures/thesis/ablations/3_adam_direction/adamdir_dqn_atari5_step.png right side b
we see that the chosen step sizes are roughly $10^5$ times larger in Name This Game when step size selection is performed directly with $u_k$. Interestingly, this is the opposite effect of what has been observed when combining globalization strategies and momentum-based optimizers in supervised learning, where frequent ascent directions result all step sizes being rejected \tocite{paper that does ls with amsgrad}. 

Investigating further, Figure % todo: ref to fig figures/thesis/ablations/3_adam_direction/adamdir_dqn_name_this_game_stepsize_loss.png
...
Turning PPO in Brax, we observe that performance is also greatly improved for all methods when using $\hat{u}_k$ in place of $u_k$ when selecting step size, as shown in Figure % todo: ref to figures/thesis/ablations/3_adam_direction/adamdir_ppo_mujoco_step.png a (left)
. 
\begin{figure}[htbp]
    \centering
...
\end{figure}
In Figure % todo: ref fig figures/thesis/ablations/3_adam_direction/adamdir_ppo_mujoco_step.png b
we see the actor step sizes for the line search are much larger than the tuned baseline when using $u_k$, while they are within an order of magnitude with $\hat{u}_k$. For GRASS and the trust region methods, the actor step sizes are consistent between the two variants, but the critic step size is larger when using momentum in selection. Figure % todo: ref fig figures/thesis/ablations/3_adam_direction/adamdir_ppo_humanoid_stepsize_loss.png
...
\end{figure}
\todo{note for Adam: didn't run other methods to save compute, seems like bad practice so maybe run regular TR too? note i run them all for PPO mujoco bc its cheaper} Note that a gain-ratio window of $1$ corresponds to updating the step size after every update using the single-step ratio, while the larger window matches the $K$ used in the main experiments. We observe that performance is much improved with $K$ set to 500, which corresponds to $1/4$ of the target network update interval, suggesting improved robustness to noisy gradient estimates in determining the true accuracy of the first-order model. Examining the step sizes for Name this Game in % todo: ref fig figures/thesis/ablations/4_gain_ratio_window/gain_ratio_window_dqn_atari_step.png right side b 
we see the single-step ratio results in larger accepted step size and worse performance, similar to the effect of small batch size. 

Interestingly, this effect does not hold for PPO in Brax, where trust-region-based methods appear compatible with the single-step gain ratio. Figure % todo: ref to figures/thesis/ablations/4_gain_ratio_window/gain_ratio_window_ppo_mujoco_step.png left side a
...
We also observe that step sizes remain largely unaffected by this change, as demonstrated for Humanoid in Figure % todo: ref to fig figures/thesis/ablations/4_gain_ratio_window/gain_ratio_window_ppo_mujoco_step.png right side b
...
\begin{figure}[htbp]
    ...
    \label{fig:sam_qbert}
\end{figure}
Figure % todo: ref to fig figures/thesis/ablations/1_sam/sam_qbert_step_sharpness.png left side a
compares the performance of PALS and GRASS in Qbert with and without SAM, an environment in which both methods previously failed to learn. When using SAM, the adaptive methods are competitive with the tuned baselines, whose performance is also slightly improved. Figure % todo: ref fig figures/thesis/ablations/1_sam/sam_qbert_step_sharpness.png left side b
shows that when using SAM the chosen step sizes are much larger, within an order of magnitude below or above the tuned baseline. As a result, we see that sharpness is also decreased and now closely matches that of Tuned-32 for both the adaptive methods and Tuned-512. To confirm the effectiveness of SAM in this enviornment, Figure % todo: ref figures/thesis/ablations/1_sam/sam_qbert_stepsize_loss.png
examines the loss along the update direction for a single update with PALS in Qbert.
We find the loss landscape is indeed smoother when using SAM, allowing PALS to accept a step size with a larger resulting displacement. 

The steady increase in step size when using SAM for both methods is concerning, as it resembles the pattern of the failure modes observed in Section~\ref{sec:dqn_results}
where some methods suffered from performance degradation over the course of the run. 
Indeed, examining Figure % todo: ref fig figures/thesis/ablations/1_sam/sam_atari5_return_step.png left sida a
we see that while performance across the Atari-5 is improved for PALS and Tuned-512, performance for GRASS is unchanged due to worsened behavior in some environments, as shown in Battle Zone in Figure % todo: ref fig figures/thesis/ablations/1_sam/sam_atari5_return_step.png right side b
. 
. 
\begin{figure}[htbp]
    \centering
...
\end{figure}
In this environment, we now observe the steadily increasing step size and corresponding performance degradation as seen with other globalization strategies in Section~\ref{sec:dqn_results}. This finding suggests that selecting step size based on the SAM objective can lead to aggressively large steps for the true objective. To address this issue with GRASS, one can increase the growth threshold $T_{\text{hi}}$ in the gain-ratio
as shown in figure % todo: ref fig figures/thesis/ablations/1_sam/sam_atari5_return_step.png left sida a
which improves performance by making the method more conservative. However, in practice we found that this change to be overfit to Atari, harming performance in classic control. 

The improved performance of Tuned-512 with SAM supports that the loss of the implicit regularization afforded by small batch size is harmfull with DQN in Atari. Conversely, tuned-32 fails with SAM, \todo{Note for Adam: Think this could be because I didn't retune with SAM, currently running this before i make a comment}
